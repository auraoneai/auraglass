import{r as g,b as Be,d as L,j as e,m as N}from"./iframe-DinEdlu4.js";import{u as Ee}from"./useMotionPreference-Bqsl3944.js";import{u as Qe}from"./a11y-BZVU29oS.js";import{c as Ge}from"./createGlassStyle-BfWnO-qv.js";import{u as Ze}from"./soundDesign-BHpxgFtq.js";import{O as Je}from"./OptimizedGlassCore-mTd-BSmd.js";import"./preload-helper-PPVm8Dsz.js";import"./deviceCapabilities-8hOeRztp.js";const Ke=[{id:"stylegan2-faces",name:"StyleGAN2 Faces",description:"High-quality human face generation",type:"stylegan",category:"faces",resolution:1024,latentDim:512,trained:!0},{id:"dcgan-art",name:"DCGAN Art",description:"Abstract art generation",type:"dcgan",category:"art",resolution:256,latentDim:100,trained:!0},{id:"biggan-objects",name:"BigGAN Objects",description:"Conditional object generation",type:"biggan",category:"objects",resolution:512,latentDim:128,trained:!0},{id:"cyclegan-style",name:"CycleGAN Style Transfer",description:"Unpaired image-to-image translation",type:"cyclegan",category:"style_transfer",resolution:256,latentDim:256,trained:!0},{id:"progressive-landscapes",name:"Progressive Landscapes",description:"Landscape scene generation",type:"progressive",category:"landscapes",resolution:512,latentDim:256,trained:!0},{id:"stylegan3-general",name:"StyleGAN3 General",description:"General purpose image generation",type:"stylegan",category:"general",resolution:512,latentDim:512,trained:!1}],Xe={seed:42,truncation:.7,styleStrength:1,noiseStrength:.5,batchSize:4,interpolationSteps:10},Ye={epochs:100,batchSize:32,learningRate:2e-4,beta1:.5,beta2:.999,discriminatorSteps:1,generatorSteps:1},ea={"--glass-text-primary":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))","--typography-text-primary":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))","--glass-theme-text":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))",color:"var(--glass-theme-text, rgba(255, 255, 255, 0.95))"},te=g.forwardRef(({availableModels:s=Ke,selectedModel:ve="stylegan2-faces",generationParams:Ne={},trainingConfig:je={},showModelSelector:Ce=!0,showGenerationControls:Ae=!1,showTrainingControls:ge=!1,showLatentSpace:Pe=!1,showProgress:Ie=!0,enableInterpolation:de=!0,enableRealTime:ne=!1,maxGenerations:C=16,canvasWidth:aa=256,canvasHeight:sa=256,onModelSelect:me,onGenerate:ue,onTrainingProgress:pe,className:Le="",...Te},ze)=>{const re=Be(),[v,he]=g.useState(!1),[A,fe]=g.useState(!1),[j,ke]=g.useState(ve),[oe,De]=g.useState([]),[y,Re]=g.useState([]),[le,He]=g.useState({epoch:0,generatorLoss:0,discriminatorLoss:0}),[be,qe]=g.useState([]),[ye,ie]=g.useState(0),[d,P]=g.useState({...Xe,...Ne}),[xe,ta]=g.useState({...Ye,...je});g.useRef([]);const Se=g.useRef(null);g.useRef(null),Qe("glass-gan-generator");const{shouldAnimate:w}=Ee(),{play:M}=Ze(),h=s.find(a=>a.id===j)||s[0],we=g.useCallback(a=>Array.from({length:a},()=>(Math.random()-.5)*2),[]),I=g.useCallback((a,t,m)=>{const r=document.createElement("canvas");r.width=t.resolution,r.height=t.resolution;const p=r.getContext("2d");if(!p)return"";const S=p.createImageData(r.width,r.height),n=S.data;switch(t.type){case"stylegan":for(let c=0;c<r.height;c++)for(let l=0;l<r.width;l++){const o=(c*r.width+l)*4,f=a.slice(0,8).reduce((u,x)=>u+x,0)/8,b=(l/r.width-.5)*2,G=(c/r.height-.5)*2;if(t.category==="faces"){const u=Math.exp(-(b*b+G*G*.8)*2),x=Math.sin(b*10)*Math.cos(G*10)*.1;n[o]=Math.floor((.8+f*.2+x)*u*255),n[o+1]=Math.floor((.7+f*.3+x)*u*255),n[o+2]=Math.floor((.6+f*.4+x)*u*255),n[o+3]=255}else{const u=Math.sin(b*5+f)*Math.cos(G*5+f);n[o]=Math.floor((u+1)*128),n[o+1]=Math.floor((Math.sin(u*2)+1)*128),n[o+2]=Math.floor((Math.cos(u*3)+1)*128),n[o+3]=255}}break;case"dcgan":for(let c=0;c<r.height;c++)for(let l=0;l<r.width;l++){const o=(c*r.width+l)*4,b=.02*(1+a.slice(0,5).reduce((_e,$e)=>_e+$e,0)*.1),G=Math.sin(l*b)*Math.cos(c*b),u=Math.sin((l+c)*b*.8),x=Math.cos((l-c)*b*1.2);n[o]=Math.floor((G+1)*128),n[o+1]=Math.floor((u+1)*128),n[o+2]=Math.floor((x+1)*128),n[o+3]=255}break;case"biggan":for(let c=0;c<r.height;c++)for(let l=0;l<r.width;l++){const o=(c*r.width+l)*4,f=a.slice(-10),b=Math.floor(Math.abs(f[0])*10),u=[[255,100,100],[100,255,100],[100,100,255],[255,255,100],[255,100,255],[100,255,255],[200,150,100],[150,100,200],[100,200,150],[180,180,180]][b]||[128,128,128],x=(Math.random()-.5)*50;n[o]=Math.max(0,Math.min(255,u[0]+x)),n[o+1]=Math.max(0,Math.min(255,u[1]+x)),n[o+2]=Math.max(0,Math.min(255,u[2]+x)),n[o+3]=255}break;default:for(let c=0;c<r.height;c++)for(let l=0;l<r.width;l++){const o=(c*r.width+l)*4,f=a.slice(0,3).reduce((b,G,u)=>b+G*Math.sin((l+c)*.01*(u+1)),0);n[o]=Math.floor((Math.sin(f)+1)*128),n[o+1]=Math.floor((Math.cos(f*1.1)+1)*128),n[o+2]=Math.floor((Math.sin(f*.9)+1)*128),n[o+3]=255}}return p.putImageData(S,0,0),r.toDataURL()},[]),ce=g.useCallback(async()=>{if(!h.trained){M("error");return}he(!0),ie(0),M("processing");const a=[],t=[];for(let m=0;m<d.batchSize;m++){ie(m/d.batchSize*100);const p=we(h.latentDim).map(n=>n*d.truncation),S=I(p,h,d.seed+m);a.push(S),t.push(p),await new Promise(n=>setTimeout(n,L.DURATION.fast*1.3))}De(m=>[...a,...m].slice(0,C)),Re(m=>[...t,...m].slice(0,C)),ie(100),ue?.(a,d),he(!1),M("success")},[h,d,we,I,C,ue,M]),We=g.useCallback(async()=>{if(y.length<2)return;const a=y[0],t=y[1],m=d.interpolationSteps,r=[];for(let p=0;p<=m;p++){const S=p/m,n=a.map((l,o)=>l*(1-S)+t[o]*S),c=I(n,h,d.seed);r.push(c)}qe(r)},[y,d.interpolationSteps,d.seed,I,h]),Ve=g.useCallback(async()=>{if(!h)return;fe(!0),M("processing");for(let t=0;t<xe.epochs;t++){const m=1+Math.random()*.5-t*.01,r=.8+Math.random()*.3-t*.005;He({epoch:t+1,generatorLoss:Math.max(.1,m),discriminatorLoss:Math.max(.1,r)}),pe?.(t+1,{generator:m,discriminator:r}),await new Promise(p=>setTimeout(p,L.DURATION.fast))}const a=s.findIndex(t=>t.id===j);a!==-1&&(s[a].trained=!0),fe(!1),M("success")},[h,xe.epochs,j,s,pe,M]),Me=g.useCallback(()=>{const a=Se.current;if(!a||y.length===0)return;const t=a.getContext("2d");t&&(t.clearRect(0,0,a.width,a.height),y.slice(0,10).forEach((m,r)=>{const p=(m[0]+2)/4*a.width,S=(m[1]+2)/4*a.height;t.fillStyle=`hsl(${r*36}, 70%, 60%)`,t.beginPath(),t.arc(p,S,5,0,Math.PI*2),t.fill(),y.slice(r+1,10).forEach((n,c)=>{const l=Math.sqrt(Math.pow(m[0]-n[0],2)+Math.pow(m[1]-n[1],2));if(l<1){const o=(n[0]+2)/4*a.width,f=(n[1]+2)/4*a.height;t.strokeStyle=`color-mix(in srgb, var(--glass-white) ${30*(1-l)}%, transparent)`,t.lineWidth=1,t.beginPath(),t.moveTo(p,S),t.lineTo(o,f),t.stroke()}})}))},[y]),Fe=g.useCallback(a=>{ke(a),me?.(a),M("select")},[me,M]);g.useEffect(()=>{if(ne&&h.trained){const a=setInterval(()=>{v||ce()},5e3);return()=>clearInterval(a)}},[ne,h.trained,v,ce]),g.useEffect(()=>{Me()},[y,Me]);const Ue=()=>e.jsxs("div",{className:"glass-space-y-4",children:[e.jsx("h4",{className:"glass-text-sm glass-font-medium glass-text-primary-glass-opacity-80",children:"GAN Models"}),e.jsx("div",{className:"glass-grid glass-grid-cols-1 md:glass-grid-cols-2 glass-gap-3",children:s.map(a=>e.jsx(N.div,{className:`
                glass-p-3 glass-radius-lg glass-border glass-cursor-pointer glass-transition-all duration-[${L.DURATION.fast}ms]
                ${j===a.id?"glass-border-blue glass-surface-blue/20":"glass-border-white/20 hover:glass-border-white/40 glass-surface-subtle/5"}
...
}`,...se.parameters?.docs?.source}}};const ua=["Default","StyleGAN2Faces","DCGANArt","BigGANObjects","CycleGANStyleTransfer","ProgressiveGAN","UntrainedModel","HighResolution","LowResolution","RealTimeGeneration","InterpolationFocus","TrainingMode","LatentSpaceFocus","MinimalInterface","BatchGeneration","HighTruncation","LowTruncation","FaceGeneration","ArtGeneration","ObjectGeneration","LandscapeGeneration","StyleTransferMode","AllModelsAvailable","FastGeneration","SlowHighQuality"];export{ee as AllModelsAvailable,J as ArtGeneration,B as BatchGeneration,D as BigGANObjects,R as CycleGANStyleTransfer,k as DCGANArt,T as Default,Z as FaceGeneration,ae as FastGeneration,W as HighResolution,E as HighTruncation,U as InterpolationFocus,X as LandscapeGeneration,_ as LatentSpaceFocus,V as LowResolution,Q as LowTruncation,$ as MinimalInterface,K as ObjectGeneration,H as ProgressiveGAN,F as RealTimeGeneration,se as SlowHighQuality,z as StyleGAN2Faces,Y as StyleTransferMode,O as TrainingMode,q as UntrainedModel,ua as __namedExportsOrder,ma as default};
