import{r as o,b as Ke,d as he,j as e,m as T}from"./iframe-DinEdlu4.js";import{u as Qe}from"./useMotionPreference-Bqsl3944.js";import{u as Ze}from"./a11y-BZVU29oS.js";import{c as ea}from"./createGlassStyle-BfWnO-qv.js";import{u as aa}from"./soundDesign-BHpxgFtq.js";import{O as sa}from"./OptimizedGlassCore-mTd-BSmd.js";import"./preload-helper-PPVm8Dsz.js";import"./deviceCapabilities-8hOeRztp.js";const ta=[{id:"conv2d_1",name:"Early Features",description:"Basic edges and textures",type:"conv",depth:1,features:["edges","lines","basic_shapes"],strength:.5},{id:"conv2d_5",name:"Texture Patterns",description:"Complex textures and patterns",type:"conv",depth:5,features:["textures","patterns","repetition"],strength:.7},{id:"mixed3a",name:"Object Parts",description:"Parts of objects and shapes",type:"inception",depth:10,features:["object_parts","curves","complex_shapes"],strength:1},{id:"mixed4a",name:"Abstract Objects",description:"Abstract object representations",type:"inception",depth:15,features:["abstract_objects","compositions","spatial_relations"],strength:1.2},{id:"mixed4d",name:"Complex Structures",description:"Complex architectural structures",type:"inception",depth:18,features:["buildings","architecture","complex_structures"],strength:1.5},{id:"mixed5b",name:"High-Level Concepts",description:"Abstract concepts and scenes",type:"inception",depth:25,features:["scenes","concepts","abstract_ideas"],strength:2}],ra={layers:["mixed3a"],iterations:20,learningRate:.01,octaveScale:1.4,octaves:4,maxLoss:10,stepSize:1.5,tileSize:512},na={"--glass-text-primary":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))","--typography-text-primary":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))","--glass-theme-text":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))",color:"var(--glass-theme-text, rgba(255, 255, 255, 0.95))"},te=o.forwardRef(({imageSource:n,availableLayers:re=ta,selectedLayers:ze=["mixed3a"],dreamSettings:Me={},showLayerSelector:ve=!1,showPreview:_e=!0,showSettings:ye=!1,enableRealTime:ne=!1,enableAnimation:N=!0,enableTiling:We=!0,animationSpeed:be=1,canvasWidth:xe=800,canvasHeight:fe=600,onDreamGenerated:we,onLayerActivation:Se,onProgress:Le,className:Ie="",...De},Ce)=>{const Oe=Ke(),[b,Re]=o.useState(!1),[Ne,je]=o.useState(0),[ie,Ae]=o.useState(0),[f,Fe]=o.useState(n||""),[j,Ge]=o.useState(""),[Te,qe]=o.useState({}),Pe=o.useRef(0),[Ve,Ee]=o.useState(We),[$e,Ue]=o.useState(N),[t,w]=o.useState({...ra,...Me,layers:ze}),le=o.useRef(null),oe=o.useRef(null),P=o.useRef();Ze("glass-deep-dream");const{shouldAnimate:x}=Qe(),{play:S}=aa(),He=o.useCallback((a,s)=>{const{width:i,height:c}=s,d=[];switch(a.type){case"conv":for(let l=0;l<100;l++){const g=Math.floor(Math.random()*i),v=(Math.floor(Math.random()*c)*i+g)*4,u=s.data[v],p=s.data[v+1],h=s.data[v+2],y=Math.tanh((u+p+h)/765-.5)*a.strength;d.push(y)}break;case"inception":for(let l=0;l<50;l++){const g=(Math.random()-.5)*a.strength*2;d.push(Math.tanh(g))}break;default:for(let l=0;l<64;l++)d.push((Math.random()-.5)*a.strength)}return d},[]),ke=o.useCallback((a,s,i)=>{const c=new Uint8ClampedArray(a.data),{width:d,height:l}=a;for(let g=0;g<l;g++)for(let m=0;m<d;m++){const v=(g*d+m)*4;let u=c[v],p=c[v+1],h=c[v+2];switch(s.type){case"conv":const y=m/d-.5,L=g/l-.5,R=Math.sqrt(y*y+L*L),A=Math.sin(R*20)*i*s.strength*50;u=Math.max(0,Math.min(255,u+A)),p=Math.max(0,Math.min(255,p+A)),h=Math.max(0,Math.min(255,h+A));break;case"inception":const ge=Math.cos((m+g)*.05)*Math.sin(m*.02),Je=Math.sin((m+g)*.05)*Math.cos(g*.02),me=(ge+Je)*i*s.strength*30;u=Math.max(0,Math.min(255,u+me)),p=Math.max(0,Math.min(255,p+me*.8)),h=Math.max(0,Math.min(255,h+me*.6));break;case"dense":const ue=Math.sin(m*.01)*Math.cos(g*.01)*i*s.strength*20;u=Math.max(0,Math.min(255,u*(1+ue/255))),p=Math.max(0,Math.min(255,p*(1+ue/255))),h=Math.max(0,Math.min(255,h*(1+ue/255)));break;default:const pe=Math.sin(m*.03)*Math.cos(g*.03)*i*s.strength*15;u=Math.max(0,Math.min(255,u+pe)),p=Math.max(0,Math.min(255,p+pe)),h=Math.max(0,Math.min(255,h+pe))}c[v]=u,c[v+1]=p,c[v+2]=h}return new ImageData(c,d,l)},[]),ce=o.useCallback(async()=>{if(!f)return;Re(!0),je(0),Ae(0),S("processing");const a=le.current,s=oe.current;if(!a||!s)return;const i=a.getContext("2d"),c=s.getContext("2d");if(!i||!c)return;const d=new Image;d.onload=async()=>{i.drawImage(d,0,0,a.width,a.height);let l=i.getImageData(0,0,a.width,a.height);for(let m=0;m<t.octaves;m++){const v=Math.pow(t.octaveScale,m);Math.floor(a.width/v),Math.floor(a.height/v);for(let u=0;u<Math.floor(t.iterations/t.octaves);u++){const p=m*Math.floor(t.iterations/t.octaves)+u;je(p);const h=p/t.iterations*100;Ae(h),Le?.(h,p);for(const y of t.layers){const L=re.find(R=>R.id===y);if(L){const R=He(L,l);qe(ge=>({...ge,[y]:R})),Se?.(y,R);const A=t.learningRate*t.stepSize;l=ke(l,L,A)}}c.putImageData(l,0,0),await new Promise(y=>setTimeout(y,he.DURATION.fast/be))}}const g=s.toDataURL();Ge(g),we?.(g,t),Re(!1),S("success")},d.src=f},[f,t,re,He,ke,be,Le,Se,we,S]),de=o.useCallback(()=>{if(!N||b)return;Pe.current+=1;const a=oe.current;if(a&&j){const s=a.getContext("2d");if(s){const i=new Image;i.onload=()=>{s.save(),s.globalAlpha=.1,s.translate(a.width/2,a.height/2),s.rotate(Pe.current*.01),s.drawImage(i,-a.width/2,-a.height/2),s.restore()},i.src=j}}P.current=requestAnimationFrame(de)},[N,b,j]),Be=o.useCallback(a=>{w(s=>{const i=s.layers.includes(a)?s.layers.filter(c=>c!==a):[...s.layers,a];return{...s,layers:i}}),S("select")},[S]);o.useEffect(()=>{if(!(!N||b))return P.current=requestAnimationFrame(de),()=>{P.current&&cancelAnimationFrame(P.current)}},[N,b,de]),o.useEffect(()=>{if(ne&&f){const a=setTimeout(()=>{ce()},1e3);return()=>clearTimeout(a)}},[t,ne,f,ce]);const Xe=()=>e.jsxs("div",{className:"glass-space-y-4",children:[e.jsx("h4",{className:"glass-text-sm glass-font-medium glass-text-primary-glass-opacity-80",children:"Neural Layers"}),e.jsx("div",{className:"glass-space-y-2",children:re.map(a=>e.jsxs(T.div,{className:`
                p-3 rounded-lg border cursor-pointer transition-all duration-[${he.DURATION.fast}ms]
                ${t.layers.includes(a.id)?"border-blue-400 bg-blue-400/20":"border-white/20 hover:border-white/40 bg-white/5"}
              `,whileHover:x?{scale:1.01}:{},whileTap:x?{scale:.99}:{},onClick:()=>Be(a.id),children:[e.jsxs("div",{className:"glass-flex glass-items-start glass-justify-between",children:[e.jsxs("div",{className:"glass-flex-1",children:[e.jsxs("div",{className:"glass-flex glass-items-center glass-space-x-2 glass-mb-1",children:[e.jsx("h5",{className:"glass-text-sm glass-font-medium glass-text-primary-glass-opacity-90",children:a.name}),e.jsx("span",{className:`
...
}`,...se.parameters?.docs?.source}}};const ya=["Default","EarlyLayers","DeepLayers","ConvolutionalLayers","InceptionLayers","DenseLayers","MultiLayerDream","HighIterations","LowIterations","HighLearningRate","LowLearningRate","ManyOctaves","FewOctaves","RealTimeMode","AnimationDisabled","FastAnimation","SlowAnimation","MinimalInterface","LayerSelectorOnly","SettingsOnly","PreviewOnly","LargeCanvas","SmallCanvas","NoLayersSelected","AllLayersSelected","HighIntensity","LowIntensity"];export{ee as AllLayersSelected,E as AnimationDisabled,M as ConvolutionalLayers,z as DeepLayers,H as Default,W as DenseLayers,k as EarlyLayers,$ as FastAnimation,q as FewOctaves,ae as HighIntensity,D as HighIterations,O as HighLearningRate,_ as InceptionLayers,K as LargeCanvas,X as LayerSelectorOnly,se as LowIntensity,C as LowIterations,F as LowLearningRate,G as ManyOctaves,B as MinimalInterface,I as MultiLayerDream,Z as NoLayersSelected,J as PreviewOnly,V as RealTimeMode,Y as SettingsOnly,U as SlowAnimation,Q as SmallCanvas,ya as __namedExportsOrder,va as default};
