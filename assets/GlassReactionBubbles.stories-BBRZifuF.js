import{r as u,b as xe,j as t,e as ve,m as h,c as r}from"./iframe-C4NFeGrN.js";import{u as Ce}from"./soundDesign-BwnboANF.js";import{u as be}from"./a11y-Drh7-6qm.js";import{u as je}from"./useMotionPreference-Dszu9Lq8.js";import{c as x}from"./createGlassStyle-BfWnO-qv.js";import{O as Ne}from"./OptimizedGlassCore-pFwkcNDS.js";import"./preload-helper-PPVm8Dsz.js";import"./deviceCapabilities-CcJXKEC9.js";var Me={};const Be=["❤️","😀","😂","🎉","👏","🔥","💯","⭐","👍","👎","😍","🤔","😮","😢","💪","🙌"],ne=["#FF6B6B","#4ECDC4","#45B7D1","#96CEB4","#FECA57","#FF9FF3","#54A0FF","#5F27CD","#00D2D3","#FF9F43"],Y=u.forwardRef(({width:o=600,height:i=400,reactions:oe=[],availableEmojis:y=Be,showControls:X=!0,showUserNames:ie=!0,bubbleLifetime:A=5e3,maxBubbles:J=50,gravity:K=.1,windForce:Q=.05,bounceEnabled:Z=!0,fadeOut:le=!0,soundEnabled:ee=!0,realTimeMode:z=!1,interactive:G=!0,onReactionAdd:te,onReactionClick:se,className:ce="",...ue},de)=>{const p=xe(),[f,ae]=u.useState(oe),[C,me]=u.useState(y[0]),[Se,Ee]=u.useState(!1),{play:re}=Ce();be("glass-reaction-bubbles");const{shouldAnimate:g}=je(),$=u.useRef(),b=typeof process<"u"&&Me?.JEST_WORKER_ID!==void 0;$.current||($.current=()=>Math.random());const a=$.current;u.useEffect(()=>{if(b||!z)return;const e=setInterval(()=>{if(a()<.3){const s=y[Math.floor(a()*y.length)],n=a()*(o-40)+20,m=a()*(i-40)+20;w(s,n,m,`user-${Date.now()}`,"Anonymous")}},2e3);return()=>clearInterval(e)},[z,y,o,i]),u.useEffect(()=>{if(b||f.length===0)return;const e=requestAnimationFrame(()=>{ae(s=>s.map(n=>{const m=n.life-16;if(m<=0)return null;const d={...n.velocity};d.y+=K,d.x+=(a()-.5)*Q;let l=n.x+d.x,c=n.y+d.y;return Z?((l<=0||l>=o-40)&&(d.x*=-.7,l=Math.max(0,Math.min(o-40,l))),(c<=0||c>=i-40)&&(d.y*=-.7,c=Math.max(0,Math.min(i-40,c)))):(l<-40&&(l=o),l>o&&(l=-40),c<-40&&(c=i),c>i&&(c=-40)),{...n,x:l,y:c,velocity:d,life:m}}).filter(n=>n!==null))});return()=>cancelAnimationFrame(e)},[f,K,Q,Z,o,i]);const w=u.useCallback((e,s,n,m,d)=>{const l={id:`reaction-${Date.now()}-${a()}`,emoji:e,userId:m||"current",userName:d||"You",userColor:ne[Math.floor(a()*ne.length)],x:s??a()*(o-40)+20,y:n??a()*(i-40)+20,timestamp:Date.now(),size:30+a()*20,velocity:{x:(a()-.5)*4,y:(a()-.5)*4-2},life:A,maxLife:A};ae(c=>[...c,l].slice(-J)),ee&&re("notification"),te?.(e,l.x,l.y)},[o,i,A,J,ee,re,te]),pe=u.useCallback(e=>{if(!G)return;const s=e.currentTarget.getBoundingClientRect(),n=e.clientX-s.left,m=e.clientY-s.top;w(C,n,m)},[G,C,w]),ge=u.useCallback((e,s)=>{s.stopPropagation(),se?.(e);for(let n=0;n<3;n++)w(e.emoji,e.x+(a()-.5)*20,e.y+(a()-.5)*20)},[se,w]),he=e=>le?Math.max(.1,e.life/e.maxLife):1,P=e=>{const s=1-e.life/e.maxLife;return .8+Math.sin(s*Math.PI)*.4},fe=({bubble:e})=>t.jsx(h.div,{className:r("glass-absolute glass-cursor-pointer glass-select-none glass-z-10"),style:{left:e.x,top:e.y,fontSize:e.size||30},initial:{scale:0,opacity:0},animate:p?{}:{scale:P(e),opacity:he(e),rotate:Math.sin(Date.now()/1e3+e.timestamp)*10},exit:{scale:0,opacity:0,y:e.y-50},transition:g?{type:"spring",stiffness:300,damping:20}:{duration:0},onClick:s=>ge(e,s),whileHover:{scale:P(e)*1.1},whileTap:{scale:P(e)*.9},children:t.jsxs("div",{className:`
        ...
        `,initial:{opacity:0,y:20},animate:p?{}:{opacity:1,y:0},transition:g?{duration:.3}:{duration:0},children:y.map(e=>t.jsx("button",{onClick:()=>me(e),className:`
            ...
            `,children:e},e))}),W={totalReactions:f.length,recentReactions:f.filter(e=>Date.now()-e.timestamp<5e3).length,mostUsedEmoji:f.reduce((e,s)=>(e[s.emoji]=(e[s.emoji]||0)+1,e),{})},H=Object.entries(W.mostUsedEmoji).sort(([,e],[,s])=>s-e)[0],we=u.useMemo(()=>b?Array.from({length:5},(e,s)=>({left:o/6*(s+1),top:i/6*(s+1),duration:3+s*.25,delay:s*.2})):Array.from({length:5},()=>({left:a()*o,top:a()*i,duration:3+a()*2,delay:a()*2})),[i,b,a,o]);return t.jsxs(Ne,{ref:de,intensity:"subtle",className:r("glass-relative glass-overflow-hidden",ce),style:{width:o,height:i},...ue,children:[t.jsxs("div",{className:r("glass-absolute glass-inset-0 glass-cursor-crosshair"),onClick:pe,style:{width:o,height:i},children:[t.jsx(ve,{children:f.map(e=>t.jsx(fe,{bubble:e},e.id))}),we.map((e,s)=>t.jsx(h.div,{className:r("glass-absolute glass-w-2 glass-h-2 glass-surface-muted glass-radius-full"),style:{left:e.left,top:e.top},animate:p?{}:{y:[0,-20,0],opacity:[.2,.8,.2],scale:[.5,1,.5]},transition:g?{duration:e.duration,repeat:1/0,delay:e.delay,ease:"easeInOut"}:{duration:0}},s))]}),X&&t.jsx("div",{className:r("glass-absolute glass-top-4 glass-left-4 glass-z-20"),children:t.jsx(ye,{})}),t.jsx(h.div,{className:`
            absolute top-4 right-4 z-20 p-3 rounded-lg
            ${x({blur:"sm",opacity:.8}).background}
          `,initial:{opacity:0,x:20},animate:p?{}:{opacity:1,x:0},transition:g?{delay:.5}:{duration:0},children:t.jsxs("div",{className:r("glass-text-sm glass-text-secondary glass-space-y-1"),children:[t.jsxs("div",{className:r("glass-flex glass-items-center glass-space-x-2"),children:[t.jsx("span",{children:W.totalReactions}),t.jsx("span",{className:r("glass-text-muted"),children:"total"})]}),t.jsxs("div",{className:r("glass-flex glass-items-center glass-space-x-2"),children:[t.jsx("span",{children:W.recentReactions}),t.jsx("span",{className:r("glass-text-muted"),children:"recent"})]}),H&&t.jsxs("div",{className:r("glass-flex glass-items-center glass-space-x-2"),children:[t.jsx("span",{children:H[0]}),t.jsxs("span",{className:r("glass-text-muted"),children:[H[1],"x"]})]})]})}),G&&X&&t.jsxs(h.div,{className:`
...
}`,...I.parameters?.docs?.source}}};const Ie=["Default","RealTimeMode","CustomEmojis","NoControls","CompactView","LargeCanvas","HighGravity","LowGravity","NoBounce","LongLifetime","ShortLifetime","ManyBubbles","FewBubbles","NoFadeOut","StrongWind","SilentMode","ReadOnlyMode","EmojiStorm"];export{F as CompactView,M as CustomEmojis,j as Default,I as EmojiStorm,q as FewBubbles,E as HighGravity,S as LargeCanvas,k as LongLifetime,R as LowGravity,V as ManyBubbles,T as NoBounce,B as NoControls,U as NoFadeOut,_ as ReadOnlyMode,N as RealTimeMode,L as ShortLifetime,O as SilentMode,D as StrongWind,Ie as __namedExportsOrder,_e as default};
