import{f as ie}from"./index-CLSxArU-.js";import{r as c,b as Te,j as r,e as Ee,m as S}from"./iframe-CToTmdO0.js";import{u as Le}from"./soundDesign-Dn36MD7A.js";import{u as Be}from"./a11y-E_E8Udq3.js";import{u as We}from"./useMotionPreference-BhLipaJT.js";import{c as ce}from"./createGlassStyle-BfWnO-qv.js";import{O as qe}from"./OptimizedGlassCore-tBAFSalT.js";import"./index-ByImX2pa.js";import"./preload-helper-PPVm8Dsz.js";import"./deviceCapabilities-BEZRvwEn.js";const M={low:"var(--glass-gray-600)",medium:"var(--glass-color-success)",high:"var(--glass-color-warning)",peak:"var(--glass-color-danger)"},ue=["#FF6B6B","#4ECDC4","#45B7D1","#96CEB4","#FECA57","#FF9FF3","#54A0FF","#5F27CD","#00D2D3","#FF9F43"],K=c.forwardRef(({participants:n,currentUserId:h,showAvatars:le=!0,showNames:de=!0,showMuteStatus:Q=!0,showConnectionStatus:me=!0,waveformStyle:Z="bars",sensitivity:ee=1,smoothing:O=.8,colorMode:pe="participant",maxBars:w=32,animationSpeed:se=1,realTimeMode:v=!1,soundVisualization:re=!0,showVoiceActivity:z=!0,compactMode:u=!1,onParticipantClick:ge,onMuteToggle:fe,className:he="",...ve},we)=>{const ye=Te(),[xe,Se]=c.useState({}),[Me,Ae]=c.useState({}),[p,Ne]=c.useState(n),y=c.useRef(),ae=c.useRef(p),te=c.useRef({maxBars:w,smoothing:O}),{play:ne}=Le();Be("glass-voice-waveform");const{shouldAnimate:H}=We(),Ce=(e,s)=>{const a=[],t=e*ee;for(let i=0;i<s;i++){const m=i/s*Math.PI*4;let d=Math.sin(m)*t;d+=(Math.random()-.5)*t*.3,d*=Math.max(.1,1-i/s*.7),a.push(Math.max(0,Math.min(1,d)))}return a};c.useEffect(()=>{if(!v)return;const e=setInterval(()=>{Ne(s=>s.map(a=>{if(!a.isConnected)return a;const t=a.isSpeaking,i=Math.random()<.3,m=i?Math.random()*.8+.2:Math.random()*.1;return!t&&i&&re&&ne("notification"),{...a,isSpeaking:i,audioLevel:m,lastActivity:i?Date.now():a.lastActivity}}))},200);return()=>clearInterval(e)},[v,re,ne]),c.useEffect(()=>{ae.current=p},[p]),c.useEffect(()=>{te.current={maxBars:w,smoothing:O}},[w,O]),c.useEffect(()=>{if(!v)return;let e=!0;const s=()=>{if(!e)return;const{maxBars:a,smoothing:t}=te.current,i=ae.current;Se(m=>{const d={};return i.forEach(l=>{const g=m[l.id]||new Array(a).fill(0);if(l.isSpeaking&&l.audioLevel>.1){const f=Ce(l.audioLevel,a),Y=g.map((J,X)=>J*t+f[X]*(1-t));d[l.id]=Y}else d[l.id]=g.map(f=>f*.9)}),d}),y.current=requestAnimationFrame(s)};return y.current=requestAnimationFrame(s),()=>{e=!1,y.current&&cancelAnimationFrame(y.current)}},[v]),c.useEffect(()=>{z&&Ae(e=>{const s={...e},a=Date.now();return p.forEach(t=>{s[t.id]||(s[t.id]=[]);const i=s[t.id];t.isSpeaking&&i.push(a),s[t.id]=i.filter(m=>a-m<3e4)}),s})},[p,z]);const x=(e,s)=>{switch(pe){case"activity":return e.isSpeaking?e.audioLevel<.3?M.medium:e.audioLevel<.6?M.high:M.peak:M.low;case"rainbow":return ue[s%ue.length];default:return e.color}},ke=({participant:e})=>{const s=Me[e.id]||[],a=s.length/10;return r.jsxs("div",{className:"glass-flex glass-items-center glass-space-x-1",children:[r.jsx("div",{className:"glass-flex glass-space-x-0.5",children:[0,1,2,3,4].map(t=>r.jsx("div",{className:`
                  w-1 h-3 rounded-full transition-all duration-300
                  ${a>t*.2?"bg-green-400":"bg-white/20"}
                `},t))}),r.jsx("span",{className:"glass-text-xs glass-text-primary-glass-opacity-60",children:s.length>0?`${s.length}`:"0"})]})},be=({participant:e,data:s})=>r.jsx("div",{className:"glass-flex glass-items-end glass-space-x-1 glass-h-12",children:s.map((a,t)=>r.jsx(S.div,{className:"glass-bg-transparent glass-radius-full",style:{width:u?"2px":"3px",color:x(e,n.indexOf(e)),opacity:e.isSpeaking?.8:.3},animate:{height:`${Math.max(2,a*48)}px`,opacity:e.isSpeaking?.8:.3},transition:H?{duration:.1*se,ease:"easeOut"}:{duration:0}},t))}),Ie=({participant:e,data:s})=>r.jsx("svg",{width:u?120:200,height:48,className:"glass-overflow-visible",children:r.jsx("path",{d:`M 0 24 ${s.map((a,t)=>`L ${t/(s.length-1)*(u?120:200)} ${24-a*20}`).join(" ")} L ${u?120:200} 24`,fill:"none",stroke:x(e,n.indexOf(e)),strokeWidth:"2",opacity:e.isSpeaking?.8:.3})}),je=({participant:e,data:s})=>{const a=u?20:30,t=a+5,i=a+5;return r.jsx("svg",{width:(a+5)*2,height:(a+5)*2,className:"glass-overflow-visible",children:s.map((m,d)=>{const l=d/s.length*Math.PI*2-Math.PI/2,g=a*.6,f=g+m*a*.4,Y=t+Math.cos(l)*g,J=i+Math.sin(l)*g,X=t+Math.cos(l)*f,Pe=i+Math.sin(l)*f;return r.jsx("line",{x1:Y,y1:J,x2:X,y2:Pe,stroke:x(e,n.indexOf(e)),strokeWidth:"2",opacity:e.isSpeaking?.8:.3},d)})})},Ue=({participant:e,data:s})=>r.jsx("div",{className:"glass-flex glass-items-end glass-justify-center glass-space-x-px glass-h-12 glass-w-32",children:s.map((a,t)=>r.jsx(S.div,{className:"glass-bg-transparent",style:{width:`${100/s.length}%`,color:x(e,n.indexOf(e)),opacity:e.isSpeaking?.8:.3},animate:{height:`${Math.max(1,a*48)}px`},transition:H?{duration:.1*se}:{duration:0}},t))}),De=e=>{const s=xe[e.id]||new Array(w).fill(0);switch(Z){case"waves":return r.jsx(Ie,{participant:e,data:s});case"circular":return r.jsx(je,{participant:e,data:s});case"spectrum":return r.jsx(Ue,{participant:e,data:s});default:return r.jsx(be,{participant:e,data:s})}},Ve=({participant:e,index:s})=>r.jsxs(S.div,{className:`
        ...
        `,onClick:()=>ge?.(e.id),whileHover:{scale:1.02},whileTap:{scale:.98},children:[le&&r.jsxs("div",{className:"glass-relative",children:[r.jsx("div",{className:`
            ...
            `,children:e.avatar?r.jsx("img",{src:e.avatar,alt:e.name,className:"glass-w-full glass-h-full glass-radius-full glass-object-cover"}):e.name.charAt(0).toUpperCase()}),me&&r.jsx("div",{className:`
              ...
              `}),e.isSpeaking&&r.jsx(S.div,{className:"glass-absolute glass-top-1 glass--right-1 glass-w-4 glass-h-4 glass-surface-green glass-radius-full",animate:ye?{}:{scale:[1,1.2,1]},transition:H?{duration:.8,repeat:1/0}:{duration:0}})]}),r.jsxs("div",{className:"glass-flex-1 glass-min-glass-w-0",children:[de&&r.jsxs("div",{className:"glass-flex glass-items-center glass-space-x-2",children:[r.jsxs("p",{className:`
                ...
                ${u?"text-sm":"text-base"}
              `,children:[e.name,e.id===h&&" (You)"]}),Q&&e.isMuted&&r.jsx("span",{className:"glass-text-primary glass-text-xs",children:"🔇"})]}),z&&!u&&r.jsx(ke,{participant:e})]}),r.jsxs("div",{className:"glass-flex glass-items-center glass-space-x-2",children:[De(e),Q&&!u&&r.jsx("button",{onClick:a=>{a.stopPropagation(),fe?.(e.id)},className:`
...
}`,...G.parameters?.docs?.source}}};const Qe=["Default","BarWaveform","WaveWaveform","CircularWaveform","SpectrumWaveform","RealTimeMode","ActivityColorMode","RainbowColorMode","CompactMode","MinimalInterface","ActiveSpeakers","MutedUsers","DisconnectedUsers","HighSensitivity","LowSmoothing","ManyBars","FewBars","FastAnimation","SlowAnimation","SoloCall","LargeGroup","SilentMode"];export{F as ActiveSpeakers,j as ActivityColorMode,N as BarWaveform,k as CircularWaveform,D as CompactMode,A as Default,T as DisconnectedUsers,q as FastAnimation,W as FewBars,E as HighSensitivity,_ as LargeGroup,L as LowSmoothing,B as ManyBars,V as MinimalInterface,P as MutedUsers,U as RainbowColorMode,I as RealTimeMode,G as SilentMode,$ as SlowAnimation,R as SoloCall,b as SpectrumWaveform,C as WaveWaveform,Qe as __namedExportsOrder,Ke as default};
