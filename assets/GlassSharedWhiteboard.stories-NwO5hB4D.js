import{r as l,b as Re,j as r,e as Ve,m as Be}from"./iframe-B2YkWo0R.js";import{u as qe}from"./soundDesign-WmNk9eSS.js";import{u as $e}from"./a11y-Bb31ansd.js";import{u as _e}from"./useMotionPreference-BhtD6cPl.js";import{c as K}from"./createGlassStyle-BfWnO-qv.js";import{O as Ye}from"./OptimizedGlassCore-CYII0g9k.js";import"./preload-helper-PPVm8Dsz.js";import"./deviceCapabilities-DmRU0S_3.js";const We=[{id:"pen",name:"Pen",icon:"✏️"},{id:"marker",name:"Marker",icon:"🖍️"},{id:"eraser",name:"Eraser",icon:"🧽"},{id:"shape",name:"Shape",icon:"📐"}],Xe=["var(--glass-black)","#FF0000","#00FF00","#0000FF","#FFFF00","#FF00FF","#00FFFF","#FF8000","#8000FF","#FF0080"],Ge=[2,4,8,12,16,24],Q=l.forwardRef(({width:n=800,height:o=600,users:G,currentUserId:u,strokes:ce=[],backgroundColor:ee="var(--glass-white)",gridVisible:ue=!1,showUserCursors:de=!0,showToolbar:me=!0,showUserList:he=!0,showUndoRedo:ge=!0,maxStrokes:k=1e3,realTimeSync:z=!1,soundEnabled:U=!0,canDraw:se=!0,readOnly:d=!1,onStroke:x,onClear:pe,onUndo:fe,onRedo:we,onUserCursorMove:re,className:ke="",...Ue},xe)=>{const ye=Re(),y=l.useRef(null),[te,oe]=l.useState(!1),[h,P]=l.useState(null),[b,be]=l.useState("pen"),[C,Ce]=l.useState("var(--glass-black)"),[O,ve]=l.useState(4),[g,p]=l.useState(ce),[v,S]=l.useState([]),[J,T]=l.useState([]),[Z,Se]=l.useState(G),[ae,Te]=l.useState(ue),{play:D}=qe();$e("glass-shared-whiteboard");const{shouldAnimate:De}=_e(),Fe=e=>De?e:{duration:0},H=G.find(e=>e.id===u);l.useEffect(()=>{if(!z)return;const e=setInterval(()=>{Se(s=>s.map(t=>{if(t.id===u)return t;const i=(Math.random()-.5)*50,w=(Math.random()-.5)*50,m=Math.max(0,Math.min(n,t.cursorX+i)),f=Math.max(0,Math.min(o,t.cursorY+w)),F=Math.random()<.1;if(F&&Math.random()<.3){const ie={id:`stroke-${Date.now()}-${t.id}`,userId:t.id,userName:t.name,userColor:t.color,points:[{x:m,y:f},{x:m+Math.random()*20,y:f+Math.random()*20}],tool:"pen",color:t.color,size:3,opacity:.8,timestamp:Date.now(),isComplete:!0};p(Ie=>[...Ie.slice(-k+1),ie]),x?.(ie),U&&D("draw")}return re?.(t.id,m,f),{...t,cursorX:m,cursorY:f,isDrawing:F,lastActivity:Date.now()}}))},200);return()=>clearInterval(e)},[z,u,n,o,k,x,re,U,D]);const ne=l.useCallback(()=>{const e=y.current;if(!e)return;const s=e.getContext("2d");if(s){if(s.fillStyle=ee,s.fillRect(0,0,n,o),ae){s.strokeStyle="#E5E5E5",s.lineWidth=1,s.globalAlpha=.3;for(let t=0;t<=n;t+=20)s.beginPath(),s.moveTo(t,0),s.lineTo(t,o),s.stroke();for(let t=0;t<=o;t+=20)s.beginPath(),s.moveTo(0,t),s.lineTo(n,t),s.stroke()}s.globalAlpha=1,g.forEach(t=>{if(!(t.points.length<2)){s.strokeStyle=t.color,s.lineWidth=t.size,s.globalAlpha=t.opacity,s.lineCap="round",s.lineJoin="round",t.tool==="eraser"?s.globalCompositeOperation="destination-out":s.globalCompositeOperation="source-over",s.beginPath(),s.moveTo(t.points[0].x,t.points[0].y);for(let i=1;i<t.points.length;i++)s.lineTo(t.points[i].x,t.points[i].y);s.stroke()}}),s.globalAlpha=1,s.globalCompositeOperation="source-over"}},[g,ee,ae,n,o]);l.useEffect(()=>{ne()},[ne]);const Me=l.useCallback(e=>{if(d||!se)return;const s=y.current;if(!s)return;const t=s.getBoundingClientRect(),i=e.clientX-t.left,w=e.clientY-t.top;oe(!0);const m={id:`stroke-${Date.now()}-${u}`,userId:u,userName:H?.name||"Unknown",userColor:H?.color||C,points:[{x:i,y:w}],tool:b,color:C,size:O,opacity:b==="marker"?.7:1,timestamp:Date.now(),isComplete:!1};P(m),U&&D("draw")},[d,se,u,H,b,C,O,U,D]),je=l.useCallback(e=>{if(!te||!h||d)return;const s=y.current;if(!s)return;const t=s.getBoundingClientRect(),i=e.clientX-t.left,w=e.clientY-t.top,m={...h,points:[...h.points,{x:i,y:w}]};P(m),p(f=>[...f.filter(F=>F.id!==m.id),m])},[te,h,d]),le=l.useCallback(()=>{if(!h||d)return;const e={...h,isComplete:!0};p(s=>{const t=[...s.filter(i=>i.id!==e.id),e].slice(-k);return S(i=>[...i,s]),T([]),t}),x?.(e),P(null),oe(!1)},[h,d,k,x]),Le=()=>{if(v.length===0)return;const e=v[v.length-1];T(s=>[g,...s]),S(s=>s.slice(0,-1)),p(e),fe?.()},Ne=()=>{if(J.length===0)return;const e=J[0];S(s=>[...s,g]),T(s=>s.slice(1)),p(e),we?.()},Ae=()=>{S(e=>[...e,g]),T([]),p([]),pe?.()},Ee=({user:e})=>r.jsxs(Be.div,{className:"glass-absolute glass-pointer-events-none glass-z-20",style:{left:e.cursorX,top:e.cursorY,transform:"translate(-2px, -2px)"},animate:ye?{}:{scale:e.isDrawing?1.2:1,opacity:Date.now()-e.lastActivity<5e3?1:.5},transition:Fe({type:"spring",stiffness:400,damping:30}),children:[r.jsx("svg",{width:"20",height:"20",viewBox:"0 0 24 24",children:r.jsx("path",{d:"M5 3L19 12L12 14L9 21L5 3Z",fill:e.color,stroke:"white",strokeWidth:"1"})}),r.jsx("div",{className:`
        ...
        `,children:e.name})]});return r.jsx(Ye,{ref:xe,intensity:"subtle",className:`relative ${ke}`,...Ue,children:r.jsxs("div",{className:"glass-flex glass-flex-col glass-space-y-4",children:[me&&!d&&r.jsxs("div",{className:`
              ...
              ${K({variant:"default"})}
            `,children:[r.jsxs("div",{className:"glass-flex glass-items-center glass-flex-wrap glass-gap-3",children:[r.jsx("div",{className:"glass-flex glass-space-x-2",children:We.map(e=>r.jsx("button",{onClick:()=>be(e.id),className:`
                        p-2 rounded text-sm font-medium transition-colors duration-200 glass-focus glass-touch-target glass-contrast-guard
                        ${b===e.id?"bg-white/20 text-white":"text-white/70 hover:text-white hover:bg-white/10"}
                      `,title:e.name,children:e.icon},e.id))}),r.jsx("div",{className:"glass-flex glass-space-x-1",children:Xe.map((e,s)=>r.jsx("button",{onClick:()=>Ce(e),className:`
                      ...
                      `,style:{backgroundColor:e},"aria-label":`Select color ${e}`},`${e}-${s}`))}),r.jsx("div",{className:"glass-flex glass-space-x-1",children:Ge.map(e=>r.jsx("button",{onClick:()=>ve(e),className:`
...
}`,...X.parameters?.docs?.source}}};const ss=["Default","WithGrid","RealTimeCollaboration","ReadOnlyMode","MinimalInterface","SmallCanvas","LargeCanvas","DarkBackground","NoToolbar","NoUserList","SoloMode","ManyUsers","WithManyStrokes","SilentMode","LimitedStrokes","CustomBackground"];export{X as CustomBackground,R as DarkBackground,M as Default,I as LargeCanvas,W as LimitedStrokes,$ as ManyUsers,A as MinimalInterface,V as NoToolbar,B as NoUserList,N as ReadOnlyMode,L as RealTimeCollaboration,Y as SilentMode,E as SmallCanvas,q as SoloMode,j as WithGrid,_ as WithManyStrokes,ss as __namedExportsOrder,es as default};
