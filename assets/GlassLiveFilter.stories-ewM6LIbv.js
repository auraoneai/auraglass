import{r as p,b as Ke,j as r,m as W,d as Ze}from"./iframe-DinEdlu4.js";import{u as es}from"./a11y-BZVU29oS.js";import{u as ss}from"./useMotionPreference-Bqsl3944.js";import{u as as}from"./soundDesign-BHpxgFtq.js";import{n as ts}from"./colorInput-DwY-dk75.js";import{O as rs}from"./OptimizedGlassCore-mTd-BSmd.js";import"./preload-helper-PPVm8Dsz.js";import"./deviceCapabilities-8hOeRztp.js";const ns=[{id:"grayscale",name:"Grayscale",description:"Convert to black and white",category:"color",intensity:1,parameters:{strength:1}},{id:"sepia",name:"Sepia",description:"Vintage sepia tone effect",category:"vintage",intensity:.8,parameters:{warmth:.8}},{id:"blur",name:"Gaussian Blur",description:"Smooth blur effect",category:"blur",intensity:1,parameters:{radius:5}},{id:"sharpen",name:"Sharpen",description:"Enhance image details",category:"artistic",intensity:.6,parameters:{amount:.6}},{id:"brightness",name:"Brightness",description:"Adjust image brightness",category:"color",intensity:1.2,parameters:{level:1.2}},{id:"contrast",name:"Contrast",description:"Adjust image contrast",category:"color",intensity:1.3,parameters:{level:1.3}},{id:"saturation",name:"Saturation",description:"Adjust color saturation",category:"color",intensity:1.5,parameters:{level:1.5}},{id:"hue-shift",name:"Hue Shift",description:"Shift color hues",category:"color",intensity:.5,parameters:{degrees:30}},{id:"edge-detect",name:"Edge Detection",description:"Detect and highlight edges",category:"artistic",intensity:1,parameters:{threshold:.5}},{id:"emboss",name:"Emboss",description:"3D embossed effect",category:"artistic",intensity:.8,parameters:{strength:.8}},{id:"vintage",name:"Vintage Film",description:"Old film camera effect",category:"vintage",intensity:.9,parameters:{grain:.3,vignette:.5}},{id:"neon",name:"Neon Glow",description:"Cyberpunk neon effect",category:"modern",intensity:1.2,parameters:{glow:1.2,color:"var(--glass-color-secondary)"}}],is={quality:"medium",fps:30,enableGPU:!0,batchSize:4},ls={"--glass-text-primary":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))","--typography-text-primary":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))","--glass-theme-text":"var(--glass-theme-text, rgba(255, 255, 255, 0.95))",color:"var(--glass-theme-text, rgba(255, 255, 255, 0.95))"},re=p.forwardRef(({videoSource:g,imageSource:Se,availableFilters:L=ns,selectedFilters:Re=[],processingSettings:Le={},showFilterLibrary:Ne=!0,showPreview:He=!0,showControls:Te=!1,enableRealTimeProcessing:ne=!0,enableChaining:os=!0,enableCustomFilters:cs=!1,maxFilters:ie=5,canvasWidth:ue=640,canvasHeight:me=360,onFilterApply:he,onProcessingComplete:We,onError:pe,className:Me="",...Ue},Ge)=>{Ke();const[be,ve]=p.useState(!1),[w,le]=p.useState(Re),[R,N]=p.useState({}),[fe,qe]=p.useState(""),[oe,ke]=p.useState(Se||""),[gs,ds]=p.useState(0),[H,ce]=p.useState({...is,...Le}),ye=p.useRef(null),ge=p.useRef(null),de=p.useRef(),we=p.useRef(null);es("glass-live-filter");const{shouldAnimate:j}=ss(),{play:S}=as(),Fe=p.useCallback(async()=>{const a=ge.current;if(a)try{g instanceof MediaStream?a.srcObject=g:typeof g=="string"&&(a.src=g),await a.play()}catch(n){pe?.(n)}},[g,pe]),Ae=p.useCallback((a,n)=>{let e=new ImageData(new Uint8ClampedArray(a.data),a.width,a.height);return n.forEach(s=>{const t=R[s.id]||s.parameters||{};switch(s.id){case"grayscale":e=Ve(e,F(t,"strength",1));break;case"sepia":e=xe(e,F(t,"warmth",.8));break;case"blur":e=Ee(e,F(t,"radius",5));break;case"brightness":e=ze(e,F(t,"level",1.2));break;case"contrast":e=Ie(e,F(t,"level",1.3));break;case"saturation":e=Oe(e,F(t,"level",1.5));break;case"hue-shift":e=De(e,F(t,"degrees",30));break;case"edge-detect":e=Be(e,F(t,"threshold",.5));break;case"emboss":e=_e(e,F(t,"strength",.8));break;case"vintage":e=$e(e,t);break;case"neon":e=Qe(e,t);break}}),e},[R]),Ve=(a,n)=>{const e=a.data;for(let s=0;s<e.length;s+=4){const t=Math.round(.299*e[s]+.587*e[s+1]+.114*e[s+2]);e[s]=e[s]+(t-e[s])*n,e[s+1]=e[s+1]+(t-e[s+1])*n,e[s+2]=e[s+2]+(t-e[s+2])*n}return a},xe=(a,n)=>{const e=a.data;for(let s=0;s<e.length;s+=4){const t=e[s],i=e[s+1],l=e[s+2],d=Math.min(255,t*.393+i*.769+l*.189),h=Math.min(255,t*.349+i*.686+l*.168),v=Math.min(255,t*.272+i*.534+l*.131);e[s]=t+(d-t)*n,e[s+1]=i+(h-i)*n,e[s+2]=l+(v-l)*n}return a},Ee=(a,n)=>{const e=a.data,s=a.width,t=a.height,i=new Uint8ClampedArray(e),l=Math.floor(n);for(let d=l;d<t-l;d++)for(let h=l;h<s-l;h++){let v=0,u=0,C=0,x=0,c=0;for(let f=-l;f<=l;f++)for(let b=-l;b<=l;b++){const y=((d+f)*s+(h+b))*4;v+=e[y],u+=e[y+1],C+=e[y+2],x+=e[y+3],c++}const m=(d*s+h)*4;i[m]=v/c,i[m+1]=u/c,i[m+2]=C/c,i[m+3]=x/c}return new ImageData(i,s,t)},ze=(a,n)=>{const e=a.data,s=n;for(let t=0;t<e.length;t+=4)e[t]=Math.min(255,e[t]*s),e[t+1]=Math.min(255,e[t+1]*s),e[t+2]=Math.min(255,e[t+2]*s);return a},Ie=(a,n)=>{const e=a.data,s=n,t=128*(1-s);for(let i=0;i<e.length;i+=4)e[i]=Math.max(0,Math.min(255,e[i]*s+t)),e[i+1]=Math.max(0,Math.min(255,e[i+1]*s+t)),e[i+2]=Math.max(0,Math.min(255,e[i+2]*s+t));return a},Oe=(a,n)=>{const e=a.data;for(let s=0;s<e.length;s+=4){const t=.299*e[s]+.587*e[s+1]+.114*e[s+2];e[s]=t+(e[s]-t)*n,e[s+1]=t+(e[s+1]-t)*n,e[s+2]=t+(e[s+2]-t)*n}return a},De=(a,n)=>{const e=a.data;for(let s=0;s<e.length;s+=4){const t=e[s]/255,i=e[s+1]/255,l=e[s+2]/255,d=Math.max(t,i,l),h=Math.min(t,i,l),v=d-h;if(v===0)continue;let u=0;d===t?u=(i-l)/v%6:d===i?u=(l-t)/v+2:u=(t-i)/v+4,u=(u*60+n)%360,u<0&&(u+=360);const C=(d+h)/2,x=v/(1-Math.abs(2*C-1)),c=(1-Math.abs(2*C-1))*x,m=c*(1-Math.abs(u/60%2-1)),f=C-c/2;let b=0,y=0,P=0;u<60?(b=c,y=m,P=0):u<120?(b=m,y=c,P=0):u<180?(b=0,y=c,P=m):u<240?(b=0,y=m,P=c):u<300?(b=m,y=0,P=c):(b=c,y=0,P=m),e[s]=Math.round((b+f)*255),e[s+1]=Math.round((y+f)*255),e[s+2]=Math.round((P+f)*255)}return a},Be=(a,n)=>{const e=a.data,s=a.width,t=a.height,i=new Uint8ClampedArray(e.length);for(let l=1;l<t-1;l++)for(let d=1;d<s-1;d++){const h=(l*s+d)*4;let v=0,u=0;for(let c=-1;c<=1;c++)for(let m=-1;m<=1;m++){const f=((l+c)*s+(d+m))*4,b=.299*e[f]+.587*e[f+1]+.114*e[f+2],y=[[-1,0,1],[-2,0,2],[-1,0,1]],P=[[-1,-2,-1],[0,0,0],[1,2,1]];v+=b*y[c+1][m+1],u+=b*P[c+1][m+1]}const x=Math.sqrt(v*v+u*u)>n*255?255:0;i[h]=x,i[h+1]=x,i[h+2]=x,i[h+3]=e[h+3]}return new ImageData(i,s,t)},_e=(a,n)=>{const e=a.data,s=a.width,t=a.height,i=new Uint8ClampedArray(e),l=[[-2,-1,0],[-1,1,1],[0,1,2]];for(let d=1;d<t-1;d++)for(let h=1;h<s-1;h++){let v=0,u=0,C=0;for(let c=-1;c<=1;c++)for(let m=-1;m<=1;m++){const f=((d+c)*s+(h+m))*4,b=l[c+1][m+1];v+=e[f]*b,u+=e[f+1]*b,C+=e[f+2]*b}const x=(d*s+h)*4;i[x]=Math.max(0,Math.min(255,v*n+128)),i[x+1]=Math.max(0,Math.min(255,u*n+128)),i[x+2]=Math.max(0,Math.min(255,C*n+128))}return new ImageData(i,s,t)},F=(a,n,e)=>{const s=a[n];if(typeof s=="number")return s;if(typeof s=="string"){const t=Number(s);return Number.isFinite(t)?t:e}return e},$e=(a,n)=>{let e=a;e=xe(e,.7);const s=F(n,"grain",.3),t=e.data;for(let i=0;i<t.length;i+=4){const l=(Math.random()-.5)*s*255;t[i]=Math.max(0,Math.min(255,t[i]+l)),t[i+1]=Math.max(0,Math.min(255,t[i+1]+l)),t[i+2]=Math.max(0,Math.min(255,t[i+2]+l))}return e},Qe=(a,n)=>{const e=a.data,s=F(n,"glow",1.2);for(let t=0;t<e.length;t+=4)(e[t]+e[t+1]+e[t+2])/3>128&&(e[t]=Math.min(255,e[t]*s),e[t+1]=Math.min(255,e[t+1]*s),e[t+2]=Math.min(255,e[t+2]*s));return a},T=p.useCallback(()=>{const a=ye.current,n=we.current,e=ge.current;if(!a||!n)return;const s=a.getContext("2d"),t=n.getContext("2d");if(!s||!t)return;let i=null;if(e&&!e.paused)s.drawImage(e,0,0,a.width,a.height),i=s.getImageData(0,0,a.width,a.height);else if(oe){const l=new Image;l.onload=()=>{s.drawImage(l,0,0,a.width,a.height);const d=s.getImageData(0,0,a.width,a.height);Ce(d,t)},l.src=oe;return}i&&Ce(i,t),ne&&e&&!e.paused&&(de.current=requestAnimationFrame(T))},[w,ne,oe]),Ce=(a,n)=>{const e=L.filter(i=>w.includes(i.id));if(e.length===0){n.putImageData(a,0,0);return}ve(!0);const s=Ae(a,e);n.putImageData(s,0,0);const t=n.canvas.toDataURL();qe(t),We?.(t),ve(!1)},Xe=p.useCallback(a=>{if(w.length>=ie){S("error");return}le(e=>[...e,a]);const n=L.find(e=>e.id===a);n&&(N(e=>({...e,[a]:{...n.parameters}})),he?.(a,n.parameters??{}),S("select"))},[w,ie,L,he,S]),Pe=p.useCallback(a=>{le(n=>n.filter(e=>e!==a)),N(n=>{const{[a]:e,...s}=n;return s}),S("remove")},[S]),je=p.useCallback((a,n,e)=>{N(s=>({...s,[a]:{...s[a],[n]:e}}))},[]);p.useEffect(()=>(g&&Fe(),()=>{de.current&&cancelAnimationFrame(de.current)}),[g,Fe]),p.useEffect(()=>{T()},[w,R,T]);const Ye=()=>r.jsxs("div",{className:"glass-space-y-4",children:[r.jsx("h4",{className:"glass-text-sm glass-font-medium glass-text-primary-glass-opacity-80",children:"Filter Library"}),r.jsx("div",{className:"glass-grid glass-grid-cols-2 md:glass-grid-cols-3 lg:glass-grid-cols-4 glass-gap-3",children:L.map(a=>r.jsxs(W.div,{className:`
                glass-p-3 glass-radius-lg glass-border glass-cursor-pointer glass-transition-all duration-[${Ze.DURATION.fast}ms]
...
}`,...te.parameters?.docs?.source}}};const Fs=["Default","ColorFilters","ArtisticFilters","VintageFilters","BlurEffects","ModernEffects","HighQualityProcessing","LowQualityFast","MinimalInterface","PreviewOnly","FilterLibraryFocus","CustomFiltersEnabled","MaxFiltersReached","SingleFilterMode","ChainedFilters","GPUAccelerated","CPUProcessing","HighFrameRate","LowFrameRate","WideCanvas","TallCanvas","SmallCanvas","LargeCanvas","NoFiltersSelected","AllFiltersSelected"];export{te as AllFiltersSelected,G as ArtisticFilters,k as BlurEffects,X as CPUProcessing,$ as ChainedFilters,U as ColorFilters,D as CustomFiltersEnabled,M as Default,O as FilterLibraryFocus,Q as GPUAccelerated,Y as HighFrameRate,V as HighQualityProcessing,se as LargeCanvas,J as LowFrameRate,E as LowQualityFast,B as MaxFiltersReached,z as MinimalInterface,A as ModernEffects,ae as NoFiltersSelected,I as PreviewOnly,_ as SingleFilterMode,ee as SmallCanvas,Z as TallCanvas,q as VintageFilters,K as WideCanvas,Fs as __namedExportsOrder,ws as default};
