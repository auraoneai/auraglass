...
| --- | --- | --- | --- |
| Package surface audit | `node`/`npm pack` package-surface probes | Passed | Verified 2026-05-12 for the 3.1 docs patch. Runtime root exports: 804; component-like value exports: 439; Glass-prefixed component-like exports: 317; hook exports: 121; provider exports: 29; launch recipes: 10; functional sub-entrypoints: 19; token formats: 6; CLI binaries: 1. Evidence: `reports/3.1-release/package-surface-audit.md`. |
| Component inventory audit | `npm run audit:components` | Passed | Historical 3.0 certification inventory retained for audit continuity. Inventory: 356 audited entries; direct Storybook/docs/unit-test/ContrastGuard/ARIA/focus/reduced-motion coverage reported 356/356. This is not the current 3.1 package export count. Evidence: `reports/component_inventory.json`, `reports/glassmorphism-storybook-visual-certification.json`. |
| Export map audit | `npm run audit:exports` | Passed with catalog follow-ups | Source-audit report retained in `reports/public-export-audit.{json,md}`. Current package-surface counts should come from `reports/3.1-release/package-surface-audit.md`, which verifies 804 runtime root exports and 439 component-like value exports. |
| API surface audit | `npm run audit:api` | Passed with API follow-ups | Source/API report retained in `reports/api-surface-audit.{json,md}`. Current runtime/declaration package-surface counts are recorded in `reports/3.1-release/package-surface-audit.md`. |
...
| Publish dry run | `npm publish --dry-run --provenance --access public` | Passed | Ran 2026-05-12 after adding the 3.1 CLI and recipe registry, then reran after final README/CHANGELOG updates. `prepublishOnly` passed build, pack verification, and React 18/19 Next smoke. Follow-up package-surface audit for the docs patch records the publishable `aura-glass@3.1.1` tarball shape in `reports/3.1-release/package-surface-audit.md`. |
...
- Package size: 8.1 MB from `npm pack --dry-run --json`.
- Unpacked size: 42,963,730 bytes from the verified package tarball.
- Tarball file count: 2045.
- Shasum: `b191208591340278faa431e829acec6ff4d50a89`, recorded in [package-surface-audit.md](./package-surface-audit.md) for the current patch artifact.
