...
const items = [
  ['Badges and chips', 'Status, metadata, and quick filters with strong contrast.'],
  ['Tables and metrics', 'Scannable data surfaces with stable row and column sizing.'],
  ['Kanban and timelines', 'Workflow examples with internal scrolling for dense content.'],
  ['Progress and dividers', 'Small feedback primitives used throughout templates.'],
];

const DataDisplayGallery = () => <Catalog title="Data Display Gallery" kicker="Information Surfaces" items={items} />;

const Catalog = ({ title, kicker, items }: { title: string; kicker: string; items: string[][] }) => (
  <main style={styles.page}>
    <section style={styles.header}>
      <span style={styles.kicker}>{kicker}</span>
      <h1 style={styles.title}>{title}</h1>
      <p style={styles.copy}>Data-display stories prioritize legibility, realistic density, and examples that fit inside the Storybook canvas.</p>
    </section>
...
const meta: Meta<typeof DataDisplayGallery> = {
  title: 'Reference/Category Galleries/Data Display Gallery',
