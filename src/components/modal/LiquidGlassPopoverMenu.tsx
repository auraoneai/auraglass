...
export interface LiquidGlassPopoverMenuProps extends React.HTMLAttributes<HTMLDivElement> {
  open: boolean;
...
export const LiquidGlassPopoverMenu = forwardRef<HTMLDivElement, LiquidGlassPopoverMenuProps>(
  ({ open, items, onOpenChange, sourceId, className, ...props }, ref) => {
    ...
    return (
      <LiquidGlassMaterial
        ...
        material="liquid"
        radius="xl"
        className={cn("liquid-glass-popover-menu glass-min-w-56 glass-p-2", className)}
        ...
        <div role="menu" className="glass-flex glass-flex-col glass-gap-1">
          {items.map((item) => (
            <button
              ...
              className={cn("glass-flex glass-items-center glass-gap-2 glass-radius-lg glass-px-3 glass-py-2 glass-text-left", item.selected && "glass-surface-primary")}
              onClick={() => {
                item.onSelect?.();
                onOpenChange?.(false);
              }}
