...
const statusAccent: Record<NonNullable<CollaborationParticipant['presence']>, string> = {
...
export function MultiUserGlassEditor({
  ...
  const normalizeValue = (input: typeof defaultValue | typeof value): string => {
  ...
  return (
    ...
    >
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-primary">{header}</h2>
          <p className="text-sm text-primary/70">Synced presence across team members with glass awareness.</p>
        </div>
        <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs text-primary/70">
      ...
      <div className="flex flex-wrap items-center gap-3">
        {participants.map((user) => (
          <span
            key={user.id}
            className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-primary"
          ...
          >
            ...
            >
              {user.presence && <span className={cn('h-2.5 w-2.5 rounded-full', statusAccent[user.presence])} />}
            </span>
