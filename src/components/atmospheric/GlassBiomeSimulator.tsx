...
export interface GlassBiomeSimulatorProps
...
export const GlassBiomeSimulator = forwardRef<
  HTMLDivElement,
  GlassBiomeSimulatorProps
>(
  (
    {
      width = 800,
    ...
    const generateBiomeLayers = useCallback(
      (biomeType: BiomeData["type"]) => {
    ...
    const generateParticles = useCallback(
      (biomeType: BiomeData["type"], count: number) => {
        const config = biomeConfigs[biomeType];
        const newParticles: BiomeParticle[] = [];

        for (let i = 0; i < count; i++) {
          const particleType = config.particles[
            Math.floor(Math.random() * config.particles.length)
          ] as BiomeParticle["type"];
        ...
        setParticles((prevParticles) => {
          const updated = prevParticles
            .map((particle: any) => {
              ...
              return {
                ...
                x:
                  particle.x +
                  (particle.vx + windEffect) * deltaTime * animationSpeed,
          ...
          if (updated.length < 50 * particleDensity) {
            const needed = Math.min(
              5,
              Math.floor(50 * particleDensity) - updated.length
            );
    ...
    const render = useCallback(() => {
      ...
      const skyColors = getDayNightColors(
        currentBiome.timeOfDay,
        config.colors.sky
      );
            ...
            switch (element.type) {
              ...
              case "cloud":
                ...
                const radius = Math.max(
                  10,
                  Math.min(element.width, element.height) * 0.3
                );
                for (let i = -2; i <= 2; i++) {
        ...
        switch (particle.type) {
          ...
          case "cloud":
            ...
            fogGradient.addColorStop(
              0,
              `rgba(${Array.isArray(particle.color) ? `${particle.color[0]}, ${particle.color[1]}, ${particle.color[2]}` : "255, 255, 255"}, ${Math.min(0.5, particle.opacity)})`
            );
