  ...
  function MagneticButton(
    ...
    useEffect(() => {
      ...
      if (
        ...
        !announceInteractionRef.current &&
        !prefersReducedMotion()
      ) {
        announceToScreenReader(
          "Interactive magnetic button available",
          "polite"
        );
    ...
    const calculateMagneticAttraction = useCallback(
      (distanceX: number, distanceY: number, rect: DOMRect) => {
    ...
    const handlePointerMove = useCallback(
      (e: PointerEvent) => {
        ...
        const attraction = calculateMagneticAttraction(
          distanceX,
          distanceY,
          rect
        );
    ...
    const returnToCenter = useCallback(() => {
      ...
      if (
        !isHovered &&
        elementRef.current &&
        (Math.abs(position.x) > 0.01 || Math.abs(position.y) > 0.01)
      ) {
    ...
    return (
      <Motion preset="none" className={cn("glass-inline-block")}>
        <Comp
          ref={combinedRef}
          style={{ ...combinedStyle }}
