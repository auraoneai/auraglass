...
export function LivingEcosystemSimulator({
  ...
  const normalizedLayers = useMemo(() => {
    ...
    return usable.map((layer) => ({
      ...
      biodiversity: layer.biodiversity !== undefined ? Math.max(0, Math.min(1, layer.biodiversity)) : undefined,
      status: layer.status ?? (layer.health >= 75 ? 'thriving' : layer.health >= 55 ? 'stable' : layer.health >= 40 ? 'recovering' : 'critical'),
    }));
  }, [layers]);
    ...
    >
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-primary/70">Real-time biosphere telemetry across layered habitats.</p>
        </div>
        <dl className="flex gap-6 text-sm text-primary/80">
          <div>
            <dt className="uppercase tracking-wide text-xs text-primary/50">Avg. Health</dt>
            <dd className="text-lg font-semibold text-primary">{aggregate.averageHealth}%</dd>
          </div>
          <div>
            <dt className="uppercase tracking-wide text-xs text-primary/50">Bio-Diversity</dt>
            <dd className="text-lg font-semibold text-primary">
              {(aggregate.averageBiodiversity * 100).toFixed(0)}%
            </dd>
          </div>
          <div>
            <dt className="uppercase tracking-wide text-xs text-primary/50">Critical Zones</dt>
            <dd className={cn('text-lg font-semibold', aggregate.criticalZones > 0 ? 'text-red-400' : 'text-primary')}>
        ...
        {normalizedLayers.map((layer) => {
          ...
          return (
            <button
              ...
              className={cn(
                'w-full text-left glass-radius-2xl border glass-border-soft p-4 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400',
                statusAccent[layer.status ?? 'stable'],
                isCritical ? 'shadow-[0_0_30px_-10px_rgba(248,113,113,0.7)]' : 'hover:border-white/40',
                isCritical ? 'ring-1 ring-red-400/40' : undefined,
              )}
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-lg font-medium text-primary">{layer.name}</h3>
                  <p className="text-xs text-primary/60 max-w-xs">{layer.description}</p>
                </div>
              ...
              <div className="mt-4 space-y-3">
                <div>
                  <div className="flex items-center justify-between text-xs text-primary/60">
                    ...
                    <span className={cn('font-semibold', isCritical ? 'text-red-300' : 'text-primary')}>{layer.health}%</span>
                  </div>
                  <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-white/10">
                    <div
                      className={cn('h-full rounded-full transition-all', isCritical ? 'bg-red-400/80' : 'bg-cyan-400/80')}
                ...
                {typeof layer.trend === 'number' && (
                  <div className="text-xs text-primary/60">
                    Trend: <span className={layer.trend >= 0 ? 'text-emerald-300' : 'text-red-300'}>{layer.trend >= 0 ? '+' : ''}{layer.trend}</span> pts
                  </div>
                )}
              </div>
            </button>
