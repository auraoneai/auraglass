...
export class LiquidGlassGPUDriver {
  ...
  private updateUniforms(params: {
    ...
    if (this.uniforms.uEnableRefraction) {
      this.gl.uniform1i(
        this.uniforms.uEnableRefraction,
        params.enableRefraction ? 1 : 0
      );
    }
    if (this.uniforms.uEnableReflection) {
      this.gl.uniform1i(
        this.uniforms.uEnableReflection,
        params.enableReflection ? 1 : 0
      );
    }
    if (this.uniforms.uEnableParallax) {
      this.gl.uniform1i(
        this.uniforms.uEnableParallax,
        params.enableParallax ? 1 : 0
      );
    }
...
export const LiquidGlassGPURenderer: React.FC<LiquidGlassGPUProps> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "glass-surface-primary glass-relative glass-overflow-hidden",
        className
      )}
    >
      <canvas
        ref={canvasRef}
        className={cn(
          "glass-absolute glass-inset-0 glass-w-full glass-h-full glass-pointer-events-none"
        )}
        style={{ zIndex: 1 }}
      />
