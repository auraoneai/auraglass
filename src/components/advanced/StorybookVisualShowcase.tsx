...
export type StorybookVisualKind =
  | "advanced"
  | "effect"
  | "media"
  | "visualization";

interface StorybookVisualShowcaseProps {
  name: string;
  kind?: StorybookVisualKind;
  summary?: string;
}
...
> = {
  advanced: {
    ...
    title: "Liquid glass control surface",
    summary:
      "A full-bleed, responsive inspection scene with clear material layers, roomy controls, and light-first contrast.",
    metrics: ["Latency 18 ms", "Clarity 94", "Sync 3 lanes"],
    status: ["Adaptive", "Reduced motion", "Contrast AA"],
  },
  effect: {
    eyebrow: "Visual effects",
    title: "Canvas and motion preview",
    summary:
      "A nonblank effects stage with stable dimensions, visible depth cues, and readable overlays.",
    metrics: ["Frames stable", "Canvas ready", "Blend 0.72"],
    status: ["GPU safe", "Fallback visible", "No clipping"],
  },
  media: {
    eyebrow: "Media system",
    title: "Clear media playback shell",
    summary:
      "Custom glass controls are shown over deterministic artwork with no native media controls or broken remote assets.",
    metrics: ["00:42", "Lossless", "Queue 8"],
    status: ["Custom controls", "Transcript", "Poster loaded"],
  },
  visualization: {
    eyebrow: "Visualization",
    title: "Analytic glass dashboard",
    summary:
      "Dense chart information is spaced for desktop and mobile with high-contrast labels and bounded panels.",
    metrics: ["Revenue 64k", "Signal +18%", "Variance 2.4"],
    status: ["SVG paint", "Legend clear", "Tooltip ready"],
  },
};

const swatches = ["#0ea5e9", "#14b8a6", "#f59e0b", "#e11d48"];

export function StorybookVisualShowcase({
  name,
  kind = "advanced",
  summary,
}: StorybookVisualShowcaseProps) {
            ...
            <div className="ag-story-visual-grid">
              {copy.metrics.map((metric, index) => (
                <div key={metric} className="ag-story-metric">
                  ...
                  <small>{["Primary", "Secondary", "Tertiary"][index]}</small>
                </div>
              ))}
            </div>
          </div>
