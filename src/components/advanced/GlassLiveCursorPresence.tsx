          ...
          >
            <div
              className={cn(
                "glass-radius-full",
                glassEffect &&
                  "OptimizedGlass intensity={0.2} glassBlur={6} glass-blur-sm"
              )}
              style={{
                width: size * 0.5,
                height: size * 0.5,
        ...
        animate={
          prefersReducedMotion
            ...
            : {
                ...
                y: cursor.position.y,
                rotate:
                  (Math.atan2(cursor.velocity.y, cursor.velocity.x) * 180) /
                  Math.PI,
              }
        }
        exit={{ opacity: 0, scale: 0 }}
      ...
      >
        ...
        <div
          className={cn(
            "relative",
            glassEffect &&
              "OptimizedGlass intensity={0.2} glassBlur={6} glass-blur-sm glass-elev-2"
          )}
          style={{
            width: size,
            height: size,
...
export function useLiveCursorPresence(roomId: string, currentUser: CursorUser) {
  ...
  const sendReaction = useCallback(
    (reaction: string, position: { x: number; y: number }) => {
