...
const DEFAULT_GESTURES: GestureDefinition[] = [
  ...
  },
  {
    ...
    description: 'Alternating infinity loop signalling intent to switch contexts.',
...
export function MultiDimensionalGestureRecognizer({
  ...
  const handleRecognize = useCallback((gesture: GestureDefinition) => {
  ...
  return (
    ...
    >
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-primary">Multi-dimensional Gesture Recognizer</h2>
          <p className="text-sm text-primary/70">
            Analyse six-axis motion patterns with adaptive sensitivity tuning.
          </p>
        </div>
        {showConfidence && activeGestureId && (
          <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs text-primary/80">
            Active gesture: {summarizedGestures.find((g) => g.id === activeGestureId)?.name ?? '—'}
          </span>
        )}
      </header>
        ...
        {summarizedGestures.map((gesture) => (
          <button
            ...
            className={cn(
              ...
              'hover:border-white/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400',
              activeGestureId === gesture.id ? 'border-sky-400/60 shadow-[0_0_30px_-10px_rgba(125,211,252,0.8)]' : undefined,
            )}
          >
            <div>
              <h3 className="text-lg font-medium text-primary">{gesture.name}</h3>
              <p className="text-xs text-primary/60">{gesture.description}</p>
            </div>

            <dl className="grid gap-2 text-xs text-primary/70">
              <div className="flex items-center justify-between">
                <dt className="uppercase tracking-wide">Pattern</dt>
                <dd className="font-semibold text-primary/85">{gesture.pattern}</dd>
              </div>
              {showConfidence && (
                <div className="flex items-center justify-between">
                  <dt className="uppercase tracking-wide">Baseline confidence</dt>
                  <dd className="font-semibold text-primary">
                    {(Math.min(0.99, Math.max(0, gesture.baselineConfidence ?? 0.5)) * 100).toFixed(0)}%
                  </dd>
                </div>
              )}
            </dl>
            ...
            <div className="mt-auto space-y-2">
              <div className="flex items-center justify-between text-xs text-primary/50">
                ...
                {!prefersReducedMotion && (
                  <span className="font-semibold text-primary/70">{gesture.dimensions.length} axes</span>
                )}
              </div>
              <ul className="grid gap-2 text-xs text-primary">
                {gesture.dimensions.map((dimension) => (
                  <li
                    key={`${gesture.id}-${dimension.axis}`}
                    className="flex items-center justify-between rounded-xl border border-white/5 bg-white/5 px-3 py-2"
                  >
                    <span className="font-medium text-primary/85">{dimensionLabel[dimension.axis]}</span>
                    <span className="text-primary/60">
                      Weight {(dimension.weight ?? 0.25).toFixed(2)} | Sens {(dimension.sensitivity ?? 0.5).toFixed(2)}
                    </span>
                  </li>
                ))}
              </ul>
