  ...
  const getOptimizedGlassStyles = useMemo(() => {
    ...
    switch (performanceMode) {
      ...
      case "high":
        backdropBlur = 20;
        shadow =
          "0 16px 64px color-mix(in srgb, var(--glass-black) 15%, transparent)";
        break;
      default:
        backdropBlur = 12;
    }
  ...
  if (!isVisible && deferRender) {
    return (
      <div
        ref={containerRef}
        className={cn(
          "glass-surface-placeholder glass-border-dashed",
          className
        )}
        style={{
          minHeight: "100px",
          ...
          border:
            "1px dashed color-mix(in srgb, var(--glass-black) 10%, transparent)",
        }}
      />
    );
  }
...
function VirtualizedContent({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "glass-virtualized-content glass-h-400 glass-overflow-y-auto"
      )}
    >
      ...
      {visibleChildren}
      <div
        style={{ height: (childArray.length - visibleRange.end) * 100 }}
      />{" "}
...
export function GlassPerformanceMonitor({
  ...
  const { performanceMode, batteryLevel, cpuLoad, gpuAcceleration } =
    useGlassPerformance();
