...
export const GlassQuantumTunnel = forwardRef<HTMLDivElement, GlassQuantumTunnelProps>(
  ...
  const prefersReducedMotion = useReducedMotion();
        ...
        quantumStates.forEach((state: any) => {
          ...
          state.connections.forEach((connectionId: any) => {
            ...
            if (Math.random() < tunnelingProb * 0.1) {
              setActiveTransitions((prev: any) => [...prev, {
                from: state.id,
                to: connectionId,
                progress: 0,
    ...
    useEffect(() => {
      setQuantumParticles((prev: any) => prev.map((particle: any) => {
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="relative">
            <canvas
              ...
              width={800}
              height={300}
              className="border border-white/20 glass-radius-lg glass-surface-dark/20"
            />
          ...
          <div className={`
            ...
            ${createGlassStyle({ opacity: 0.7 }).background}
          `}>
            <h3 className="text-sm font-semibold text-primary/90">Quantum Statistics</h3>
                ...
                <div className="text-primary/90 font-medium">
                  {quantumStates.length > 0 ? 
                    (Math.max(...quantumStates.map((s: any) => s.energy)) - 
                     Math.min(...quantumStates.map((s: any) => s.energy))).toFixed(1) 
                    : '0'} eV
                </div>
              </div>
            ...
            {activeTransitions.length > 0 && (
              <div className="space-y-1">
                <span className="text-primary/60 text-sm">Active Tunneling:</span>
                <div className="flex flex-wrap gap-1">
                  {activeTransitions.slice(-3).map((transition, index) => (
                    <div
                      key={index}
                      className="px-2 py-1 text-xs bg-pink-500/20 text-pink-300 glass-radius border border-pink-400/20"
                    >
