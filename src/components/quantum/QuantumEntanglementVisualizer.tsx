...
export function QuantumEntanglementVisualizer({
  ...
  return (
    ...
    >
      <header>
        <h2 className="text-xl font-semibold text-primary">Quantum Entanglement Visualizer</h2>
        <p className="text-sm text-primary/70">
          Monitor fidelity, phase correlation, and decoherence risk for entangled qubit pairs.
        </p>
      </header>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <div className="text-xs uppercase tracking-wide text-primary/60">Pair</div>
                  <div className="text-lg font-semibold text-primary">{pair.nodes[0]} ↔ {pair.nodes[1]}</div>
                </div>
              ...
              <dl className="mt-4 grid gap-3 text-xs text-primary/70">
                <div className="flex items-center justify-between">
                  <dt>Fidelity</dt>
                  <dd className="font-semibold text-primary/90">{(pair.fidelity * 100).toFixed(1)}%</dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt>Phase correlation</dt>
                  <dd className="font-semibold text-primary/90">{(pair.phaseCorrelation * 100).toFixed(1)}%</dd>
                </div>
                {typeof pair.latency === 'number' && (
                  <div className="flex items-center justify-between">
                    <dt>Latency</dt>
                    <dd className="font-semibold text-primary/80">{pair.latency.toFixed(1)} μs</dd>
                  </div>
                )}
              </dl>
              ...
              <div className="mt-4 h-2 w-full overflow-hidden rounded-full bg-white/10">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-sky-400/70 to-cyan-500/80"
