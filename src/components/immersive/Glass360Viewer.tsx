...
export interface Glass360ViewerProps
  ...
  (
    ...
    const renderVideoProjection = useCallback(
      (
        ...
        const sourceY =
          ((pitchRad + Math.PI / 2) / Math.PI) * video.videoHeight;
    ...
    const handleMouseMove = useCallback(
      (event: React.MouseEvent) => {
        ...
        const newPitch = Math.max(
          -90,
          Math.min(90, dragStart.pitch + deltaY * sensitivity)
        );
    ...
    const handleCanvasClick = useCallback(
      (event: React.MouseEvent) => {
        ...
        for (const hotspot of hotspots) {
          const hotspotPos = project3DTo2D(
            hotspot.x,
            hotspot.y,
            currentView,
            canvas.width,
      ...
      return (
        ...
        >
          ...
          <div className="glass-text-xs glass-text-secondary">
            {Math.round(currentView.yaw ?? 0)}° /{" "}
            {Math.round(currentView.pitch ?? 0)}° /{" "}
            {Math.round(currentView.fov ?? 75)}°
          </div>
        </OptimizedGlass>
      );
    };
        ...
        >
          ...
          >
            ...
            {debug && !isLoading && !hasError && (
              ...
              >
                <div className="glass-text-xs font-mono glass-gap-1">
                  ...
                  <div>
                    Projection: {source.projection || "equirectangular"}
                  </div>
