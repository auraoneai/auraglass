            ...
            {items.map((item, index) => (
                <React.Fragment key={item?.id}>
                    {item?.separator && (
                        <div className={cn(
                            'bg-white/20',
                            orientation === 'horizontal' ? 'w-px h-8 glass-mx-4' : 'h-px w-full glass-my-2 glass-mx-4'
                        )} />
                    )}
                    ...
                    {item?.children && openSubmenus.has(item?.id) && !collapsed && (
                        ...
                        >
                            <div className={cn(
                                ...
                                orientation === 'horizontal' && 'absolute top-full left-0 glass-mt-2 z-50'
                            )}>
                                <GlassNavigationMenu
                                    items={item?.children}
                                    orientation="vertical"
...
export const GlassNavigationMenuItem: React.FC<GlassNavigationMenuItemProps> = ({
    ...
    if (item?.separator) {
        return (
            <div className="h-px glass-surface-subtle/20 mx-4 my-2" />
        );
    }
        ...
        >
            ...
            >
                <div className="flex items-center gap-3 flex-1 min-glass-w-0">
                    ...
                    {/* Content */}
                    <div className="flex-1 min-glass-w-0 text-left">
                        <div className="flex items-center gap-2">
                            ...
                            {item?.badge && (
                                <span className="flex-shrink-0 px-2 py-0.5 glass-surface-red/20 glass-text-secondary glass-radius-full text-xs font-medium">
                                    {typeof item?.badge === 'number' && item?.badge > 99 ? '99+' : item?.badge}
                                </span>
                            )}
                        </div>
                        ...
                        {item?.description && (
                            <p className="text-primary/50 text-sm truncate glass-mt-0-5">
                                {item?.description}
                            </p>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {item?.external && (
                        <div className="w-3 h-3 text-primary/50">
                            ↗
                        </div>
                    )}
