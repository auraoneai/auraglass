...
>((props, ref) => {
  ...
  const effectiveOrientation =
    responsiveState.isMobile && responsiveOrientation === "vertical"
      ? "vertical"
      : orientation;
  ...
  const finalDisableAnimation = !!(
    propDisableAnimation ??
    (contextValue as any)?.disableAnimation ??
    isReducedMotion
  );
      ...
      setActiveTab: (index: number) => {
        if (
          ...
          const syntheticEvent = {
            currentTarget:
              tabRefs.current[index] || document.createElement("button"),
            preventDefault: () => {},
            stopPropagation: () => {},
  ...
  const handleCollapsedTabSelect = (collapsedIndex: number) => {
    ...
    if (realIndex !== -1) {
      ...
      const syntheticEvent = {
        currentTarget:
          tabRefs.current[realIndex] || document.createElement("button"),
        preventDefault: () => {},
        stopPropagation: () => {},
      } as React.MouseEvent<HTMLButtonElement>;
  ...
  const setScrollTarget = (
    position: ScrollPosition | { x: number; y: number },
    animate = true
  ) => {
    const scrollPos =
      "left" in position
    ...
    if (Math.abs(velocityX) > 0.5 || Math.abs(velocityY) > 0.5) {
      ...
      const animateScroll = () => {
        ...
        let newScrollX =
          ...
          scrollAnimationRef.current.velocity.x * dt * 60;
        let newScrollY =
          tabsRef.current.scrollTop -
          scrollAnimationRef.current.velocity.y * dt * 60;
        ...
        if (isMoving) {
          checkScrollVisibility();
          scrollAnimationRef.current.rafId =
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (orientation === "horizontal") {
        setScrollTarget(
          { x: container.scrollLeft, y: scrollPosition.y },
          false
        );
      } else {
    ...
    return (
      <div
        style={{
          ...
          transition:
            "transform 0.1s ease-out, width 0.2s ease, height 0.2s ease, opacity 0.2s ease",
        }}
      />
    );
