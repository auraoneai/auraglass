...
const meta: Meta<typeof LiquidGlassToolbar> = {
  title: 'Navigation/Liquid Glass Toolbar',
  component: LiquidGlassToolbar,
  parameters: { layout: "fullscreen", previewSurface: "app" },
};
export default meta;
type Story = StoryObj<typeof LiquidGlassToolbar>;

const toolbarStoryCss = `
...
export const Default: Story = {
  render: () => (
    <div className="liquid-toolbar-story">
      <style>{toolbarStoryCss}</style>
      <LiquidGlassToolbar
        floating
        scrollEdge="soft"
        left={<strong style={{ minWidth: 140 }}>Aura Studio</strong>}
        center={<span className="liquid-toolbar-muted">Project: Liquid launch</span>}
        groups={[{ id: "tools", items: [{ id: "save", label: "Save", primary: true }, { id: "share", label: "Share" }] }]}
        right={<span className="liquid-toolbar-muted">Profile</span>}
      />
      <section className="liquid-toolbar-content">
        <h2 style={{ margin: 0 }}>Toolbar over app content</h2>
      </section>
    </div>
  ),
};
