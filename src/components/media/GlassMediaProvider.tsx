...
interface MediaContextValue {
  ...
  analyzeSentiment: (
    mediaId: string
  ) => Promise<Record<number, "positive" | "neutral" | "negative">>;
...
const mockTranscriptionService = {
  async transcribe(mediaFile: MediaFile): Promise<TranscriptEntry[]> {
    ...
    for (
      let i = 0;
      i < Math.min(mockContent.length, Math.floor(duration / 15));
      i++
    ) {
      ...
      entries.push({
        ...
        keywords: ["media", "transcription", "AI", "technology"].slice(
          0,
...
export const MediaProvider: React.FC<{
  ...
  const getTranscriptAtTime = useCallback(
  ...
  const generateSubtitles = useCallback(
    async (mediaId: string, language: string) => {
      const transcript = transcripts[mediaId];
  ...
  const trackView = useCallback((mediaId: string, watchTime: number) => {
    setAnalytics((prev: any) => ({
      ...
      [mediaId]: {
        ...
        averageWatchTime:
          ((prev[mediaId]?.averageWatchTime || 0) + watchTime) / 2,
      },
    }));
  }, []);
  ...
  const startRecording = useCallback(
    async (type: "audio" | "video", options?: RecordingOptions) => {
      try {
