...
export interface GlassColorWheelProps
...
export const GlassColorWheel = forwardRef<HTMLDivElement, GlassColorWheelProps>(
  (
    ...
    const handleSaturationInteraction = useCallback(
      (event: React.MouseEvent<HTMLCanvasElement>) => {
        if (disabled) return;
        ...
        const brightness = Math.max(
          0,
          Math.min(100, ((canvas.height - y) / canvas.height) * 100)
        );
    ...
    return (
      ...
      >
        ...
        >
          {/* Color Wheel */}
          <div className='glass-relative glass-flex glass-justify-center'>
            <canvas
              ref={wheelRef}
              width={config.wheel}
              height={config.wheel}
              className='glass-cursor-crosshair glass-radius-full'
              onClick={handleWheelInteraction}
              onMouseDown={() => setIsDragging("wheel")}
              onMouseMove={(e) =>
                isDragging === "wheel" && handleWheelInteraction(e)
              }
            ...
            {/* Hue indicator */}
            <div
              className='glass-absolute glass-w-4 glass-h-4 glass-border-2 glass-border-white glass-radius-full glass-shadow-lg glass-pointer-events-none'
              style={{
                left: "50%",
                top: "50%",
          ...
          <div className='glass-relative'>
            <canvas
              ...
              className='glass-w-full glass-cursor-crosshair glass-radius-md glass-border glass-border-glass-border/20'
              onClick={handleSaturationInteraction}
              onMouseDown={() => setIsDragging("saturation")}
              onMouseMove={(e) =>
                isDragging === "saturation" && handleSaturationInteraction(e)
              }
              onMouseUp={() => setIsDragging(null)}
            />
            ...
            <div
              className='glass-absolute glass-w-3 glass-h-3 glass-border-2 glass-border-white glass-radius-full glass-shadow-lg glass-pointer-events-none'
              style={{
                left: `${hsv.s}%`,
                top: `${100 - hsv.v}%`,
          ...
          {showAlpha && (
            <div className='glass-relative'>
              <canvas
                ...
                onMouseMove={(e) =>
                  isDragging === "alpha" && handleAlphaInteraction(e)
                }
                onMouseUp={() => setIsDragging(null)}
              />
              ...
              {/* Alpha indicator */}
              <div
                className='glass-absolute glass-w-3 glass-h-6 glass-border-2 glass-border-white glass-radius-md glass-shadow-lg glass-pointer-events-none'
                style={{
          ...
          <div className="glass-flex glass-items-center glass-gap-2">
            <div
              className='glass-w-12 glass-h-8 glass-radius-md glass-border glass-border-glass-border/20 glass-shadow-inner'
              style={{
          ...
          {showSwatches && swatches.length > 0 && (
            <div className="glass-grid glass-grid-cols-8 glass-gap-1">
              {swatches.map((swatch, index) => (
                <Motion
                  key={index}
                  preset={
                    isMotionSafe && respectMotionPreference ? "scaleIn" : "none"
                  }
                  delay={index * 20}
