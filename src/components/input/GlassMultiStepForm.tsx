...
export type GlassMultiStepFormData = Record<string, unknown>;
export type GlassMultiStepValidationMode = "onChange" | "onSubmit" | "onBlur";

export interface GlassMultiStepComponentProps<TData = unknown> {
  data: TData;
  onChange: (data: TData) => void;
  validationMode: GlassMultiStepValidationMode;
}
...
export interface FormStep {
  ...
  component: React.ComponentType<Partial<GlassMultiStepComponentProps>>;
...
export interface GlassMultiStepFormProps
   ...
   * Initial form data
   */
  initialData?: GlassMultiStepFormData;
  /**
   * Form submission handler
   */
  onSubmit?: (data: GlassMultiStepFormData) => void | Promise<void>;
  /**
   ...
   * Step change handler
   */
  onStepChange?: (stepIndex: number, data: GlassMultiStepFormData) => void;
  /**
   * Form title
   */
  title?: string;
...
export interface FormContextType {
  currentStep: number;
  totalSteps: number;
  formData: GlassMultiStepFormData;
  errors?: Record<string, string>;
  isValid?: boolean;
  updateFormData: (stepId: string, data: unknown) => void;
  updateData?: (stepId: string, data: unknown) => void;
...
export const GlassMultiStepForm: React.FC<GlassMultiStepFormProps> = ({
  ...
  const [formData, setFormData] = useState<GlassMultiStepFormData>(initialData);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Submission errors are left to the caller-provided submit handler.
    } finally {
            ...
            {showSummary && (
              <div className="glass-mt-6 glass-p-4 glass-surface-subtle/5 glass-radius-lg">
                ...
                <div className="glass-gap-2">
                  {steps.map((step, index) => {
                    const isCompleted = isStepCompleted(index);
                    const hasData = Boolean(formData[step.id]);
    ...
    <div className={cn("space-y-6", className)}>
      {(title || description) && (
        <div className="glass-mb-6">
          ...
          {description && (
            <p className="glass-text-primary-opacity-70">{description}</p>
          )}
        </div>
      )}
      {children}
