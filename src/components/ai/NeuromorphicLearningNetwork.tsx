...
const DEFAULT_SIGNALS: NeuromorphicSignal[] = [
  ...
  { channel: 'cognitive', weights: [0.63, -0.44, 0.18, 0.27, -0.35], phase: 0.52 },
  { channel: 'motor', weights: [0.29, -0.17, 0.41, -0.22, 0.33], phase: 0.47 },
];

const clamp = (value: number, min: number, max: number) => Math.min(max, Math.max(min, value));

const computeMetrics = (signals: NeuromorphicSignal[]): NeuromorphicLearningSnapshot => {
  if (signals.length === 0) {
    return {
      energyUsage: 0,
      coherence: 0,
...
export function NeuromorphicLearningNetwork({
  ...
  const [snapshot, setSnapshot] = useState<NeuromorphicLearningSnapshot>(() => computeMetrics(signals));
  ...
  return (
    ...
    >
      <header>
        <h2 className="text-xl font-semibold text-primary">Neuromorphic Learning Network</h2>
        <p className="text-sm text-primary/70">
          Monitor coherence between quantum-inspired spikes and adaptive plasticity feedback.
        </p>
      </header>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {([
          ...
          <div key={label} className="rounded-2xl border border-white/10 bg-white/5 p-4 text-primary">
            <div className="flex items-center justify-between text-xs uppercase tracking-wide text-primary/60">
              <span>{label}</span>
              <span>{(value * 100).toFixed(0)}%</span>
            </div>
            <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-gradient-to-r from-sky-400/70 to-cyan-500/80" style={{ width: `${value * 100}%` }} />
            </div>
          </div>
        ))}
      </div>
      ...
      <section className="space-y-3">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-primary/60">Signal channels</h3>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {normalizedSignals.map((signal) => (
            <div key={signal.channel} className="rounded-2xl border border-white/10 bg-white/5 p-4 text-primary/80">
              <div className="flex items-center justify-between text-sm text-primary">
                <span className="font-semibold">{signal.channel}</span>
                <span className="text-xs text-primary/60">Weights {signal.weights.length}</span>
              </div>
              <div className="mt-3 space-y-2 text-xs">
                <div className="flex items-center justify-between text-primary/60">
                  <span>Avg Weight</span>
                  <span className="font-semibold text-primary/80">
                    {(signal.weights.reduce((sum, weight) => sum + weight, 0) / signal.weights.length).toFixed(2)}
                  </span>
                </div>
                <div className="flex items-center justify-between text-primary/60">
                  <span>Phase Alignment</span>
                  <span className="font-semibold text-primary/80">{((signal.phase ?? 0.5) * 100).toFixed(0)}%</span>
                </div>
              </div>
            </div>
          ))}
