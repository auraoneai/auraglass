...
const getElevationShadow = (level?: number) => {
  switch (level) {
    case 0:
      return "none";
    case 1:
      return "var(--aura-shadow-elevation-sm)";
    case 2:
      return "var(--aura-shadow-elevation-md)";
    case 3:
      return "var(--aura-shadow-elevation-lg)";
    case 4:
      return "var(--aura-shadow-elevation-xl)";
    default:
      return "var(--aura-shadow-elevation-md)";
  }
};
...
const ChartSubtitle: React.FC<React.HTMLAttributes<HTMLParagraphElement>> = ({
  className,
  ...props
}) => (
  <ContrastGuard>
...
const LegendColor: React.FC<LegendColorProps> = ({
  $color,
  $active = true,
  className,
  style,
  ...
  <div
    ...
    style={{
      backgroundColor: $color || COLORS.semantic.secondary,
...
const LegendLabel: React.FC<LegendLabelProps> = ({
  $active = true,
  className,
  style,
  ...props
...
const DynamicTooltip: React.FC<DynamicTooltipProps> = ({
  ...
  <div
    ...
    style={{
      ...
      boxShadow: $color
        ? `0 20px 48px color-mix(in srgb, ${$color} 35%, color-mix(in srgb, rgb(15, 23, 42) 32%, transparent))`
        : undefined,
      ...style,
    }}
    {...props}
...
const TooltipRow: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  className,
  ...props
}) => <div className={cn(chartStyles.tooltipRow, className)} {...props} />;

const TooltipLabel: React.FC<React.HTMLAttributes<HTMLSpanElement>> = ({
  className,
  style,
  ...props
}) => (
...
const ZoomControls: React.FC<ZoomControlsProps> = memo(
  ({ onZoomIn, onZoomOut, onReset, zoomLevel, $variant = "frosted" }) => {
    ...
    return (
      <div style={{ ...zoomControlsStyle }}>
        ...
        <ContrastGuard>
          <span style={{ ...zoomLevelStyle }}>{displayZoom}%</span>
        </ContrastGuard>
...
const GlassDataChartComponent = React.forwardRef<
  GlassDataChartRef,
  GlassDataChartProps
>((props, ref) => {
  const {
    ...
    interaction = {
      ...
      physics: {
        ...
        inertiaDuration: ANIMATION.DURATION.slow,
      },
    },
    legend = {
      show: true,
  ...
  const adaptedBlurStrength =
    (qualityGlassParams.blurStrength as "low" | "medium" | "high") ||
    blurStrength;
  ...
  const {
    ...
    type: isReducedMotion
      ? "none"
      : animation.physicsEnabled
        ? "spring"
        : "none",
    stiffness: qualityPhysicsParams.stiffness,
    damping:
      qualityPhysicsParams.dampingRatio *
      2 *
      Math.sqrt(qualityPhysicsParams.stiffness * qualityPhysicsParams.mass),
  ...
  const { isPanning, zoomLevel, applyZoom, resetZoom } =
    useChartPhysicsInteraction(chartRef, chartWrapperRef, {
      ...
      physics: {
        ...
        friction:
          interaction.physics?.friction ||
          qualityPhysicsParams.dampingRatio *
            2 *
            Math.sqrt(
              qualityPhysicsParams.stiffness * qualityPhysicsParams.mass
            ),
      ...
      inertiaDuration:
        interaction.physics?.inertiaDuration || ANIMATION.DURATION.slow,
      respectReducedMotion: true,
    });
  ...
  const handleDataPointClick = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if ((points?.length || 0) > 0) {
      ...
      if (getElementPhysicsOptions && dataPoint) {
        const physicsOptions = getElementPhysicsOptions(
          dataPoint,
          datasetIndex,
          dataIndex,
          chartType
  ...
  const handleChartHover = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const previousHoveredKey = hoveredPoint
        ? `${hoveredPoint.datasetIndex}_${hoveredPoint.dataIndex}`
        : null;
      ...
      if ((points?.length || 0) > 0) {
        ...
        if (
          interaction.physicsHoverEffects &&
          getElementPhysicsOptions &&
          datasets
        ) {
  ...
  const handleExportChart = useCallback(() => {
    ...
    if (exportOptions.includeTitle && (title || subtitle)) {
      ...
      if (subtitle) {
        ...
        exportContext.fillText(
          subtitle,
          exportCanvas.width / 2,
          title ? 45 * devicePixelRatio : 25 * devicePixelRatio
        );
      }
    }
    ...
    const quality =
      exportOptions.format === "jpeg" ? exportOptions.quality : undefined;
          ...
          interaction.showTooltips && (
            ...
            >
              ...
              <TooltipRow>
                ...
                <TooltipValue $highlighted>
                  {formatValue(hoveredPoint.value?.value ?? 0)}
                </TooltipValue>
              </TooltipRow>
            ...
            <GlassTooltip
              ...
              content={
                <div>
                  ...
                  {hoveredPoint.value?.extra &&
                    Object.entries(hoveredPoint.value.extra).map(
                      ([key, value]) => (
                        <div key={key}>
                          <strong>{key}</strong>:{" "}
                          {typeof value === "string" ||
