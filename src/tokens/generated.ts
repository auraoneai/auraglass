// token-lint-ignore-file: auto-generated token data includes raw color values
...
auraTokens.personas.forEach((persona) => {
...
export function getPersonaModeTokens(id: PersonaId, mode: PersonaMode) {
  ...
  if (!persona.metadata.modes.includes(mode)) {
    throw new Error(`Persona ${id} does not include mode ${mode}`);
  }
  return {
    colors: persona.colors.personas?.[mode],
    glass: persona.glass,
    motion: persona.motion,
    spacing: persona.spacing,
