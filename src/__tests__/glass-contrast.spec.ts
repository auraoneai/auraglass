...
function parseGradientStops(gradient: string): GradientStop[] {
  ...
  const stops: Array<{ color: [number, number, number, number]; position?: number }> = [];
...
function sampleGradientComposite(gradient: string): [number, number, number] | null {
...
function resolveSurfaceBaseColor(
  ...
  if (surface && typeof surface === 'object') {
    ...
    if ('base' in surface) {
      return resolveSurfaceBaseColor(surface.base, surface.overlay ?? fallbackOverlay);
    }
  }
