...
- Category: layout
- Direct Storybook story: `src/components/layouts/GlassGoldenRatioGrid.stories.tsx`
...
```tsx
import { GlassGoldenRatioGrid } from 'aura-glass';

export function GlassGoldenRatioGridExample() {
  return (
    <GlassGoldenRatioGrid aria-label="GlassGoldenRatioGrid example">
      GlassGoldenRatioGrid content
    </GlassGoldenRatioGrid>
  );
}
```
