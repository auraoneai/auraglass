...
## Overview
`div` is tracked in the AuraGlass component inventory and participates in the glassmorphism visual certification workflow. This page provides direct component documentation coverage for the inventory item and links the component to its owner story, visual certification evidence, and implementation source.

## Inventory Metadata

- Category: data-display
- Direct Storybook story: `src/components/card/div.stories.tsx`
...
```tsx
import { div } from 'aura-glass';

export function divExample() {
  return (
    <div aria-label="div example">
      div content
    </div>
  );
}
