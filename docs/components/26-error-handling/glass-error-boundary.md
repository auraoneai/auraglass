...
- Category: misc
- Direct Storybook story: `src/components/GlassErrorBoundary.stories.tsx`
...
```tsx
import { GlassErrorBoundary } from 'aura-glass';

export function GlassErrorBoundaryExample() {
  return (
    <GlassErrorBoundary aria-label="GlassErrorBoundary example">
      GlassErrorBoundary content
    </GlassErrorBoundary>
  );
}
```
