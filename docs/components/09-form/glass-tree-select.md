...
- `src/components/input/GlassTreeSelect.tsx`
...
```tsx
import { GlassTreeSelect } from 'aura-glass';

export function GlassTreeSelectExample() {
  return (
    <GlassTreeSelect aria-label="GlassTreeSelect example">
      GlassTreeSelect content
    </GlassTreeSelect>
  );
}
```
