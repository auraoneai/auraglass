...
- `src/components/input/GlassTransferList.tsx`
...
```tsx
import { GlassTransferList } from 'aura-glass';

export function GlassTransferListExample() {
  return (
    <GlassTransferList aria-label="GlassTransferList example">
      GlassTransferList content
    </GlassTransferList>
  );
}
```
