...
- `src/components/chat/GlassTypingIndicator.tsx`
...
```tsx
import { GlassTypingIndicator } from 'aura-glass';

export function GlassTypingIndicatorExample() {
  return (
    <GlassTypingIndicator aria-label="GlassTypingIndicator example">
      GlassTypingIndicator content
    </GlassTypingIndicator>
  );
}
```
