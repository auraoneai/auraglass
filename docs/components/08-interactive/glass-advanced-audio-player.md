...
```tsx
import { GlassAdvancedAudioPlayer } from 'aura-glass';

export function GlassAdvancedAudioPlayerExample() {
  return (
    <GlassAdvancedAudioPlayer aria-label="GlassAdvancedAudioPlayer example">
      GlassAdvancedAudioPlayer content
    </GlassAdvancedAudioPlayer>
  );
}
```
