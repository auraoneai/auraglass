...
```tsx
import { GlassAdvancedVideoPlayer } from 'aura-glass';

export function GlassAdvancedVideoPlayerExample() {
  return (
    <GlassAdvancedVideoPlayer aria-label="GlassAdvancedVideoPlayer example">
      GlassAdvancedVideoPlayer content
    </GlassAdvancedVideoPlayer>
  );
}
```
