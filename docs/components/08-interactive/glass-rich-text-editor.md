...
- `src/components/editor/GlassRichTextEditor.tsx`
...
```tsx
import { GlassRichTextEditor } from 'aura-glass';

export function GlassRichTextEditorExample() {
  return (
    <GlassRichTextEditor aria-label="GlassRichTextEditor example">
      GlassRichTextEditor content
    </GlassRichTextEditor>
  );
}
```
