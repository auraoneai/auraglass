...
```tsx
import { GlassMediaProvider } from 'aura-glass';

export function GlassMediaProviderExample() {
  return (
    <GlassMediaProvider aria-label="GlassMediaProvider example">
      GlassMediaProvider content
    </GlassMediaProvider>
  );
}
```
