...
- Category: misc
- Direct Storybook story: `src/components/interactive/GlassColorWheel.stories.tsx`
...
```tsx
import { GlassColorWheel } from 'aura-glass';

export function GlassColorWheelExample() {
  return (
    <GlassColorWheel aria-label="GlassColorWheel example">
      GlassColorWheel content
    </GlassColorWheel>
  );
}
```
