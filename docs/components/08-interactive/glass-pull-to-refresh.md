...
- `src/components/mobile/GlassPullToRefresh.tsx`
...
```tsx
import { GlassPullToRefresh } from 'aura-glass';

export function GlassPullToRefreshExample() {
  return (
    <GlassPullToRefresh aria-label="GlassPullToRefresh example">
      GlassPullToRefresh content
    </GlassPullToRefresh>
  );
}
```
