...
```tsx
import { ContextAwareGlass } from 'aura-glass';

export function ContextAwareGlassExample() {
  return (
    <ContextAwareGlass aria-label="ContextAwareGlass example">
      ContextAwareGlass content
    </ContextAwareGlass>
  );
}
```
