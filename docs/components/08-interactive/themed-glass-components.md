...
- Category: misc
- Direct Storybook story: `src/components/interactive/ThemedGlassComponents.stories.tsx`
...
```tsx
import { ThemedGlassComponents } from 'aura-glass';

export function ThemedGlassComponentsExample() {
  return (
    <ThemedGlassComponents aria-label="ThemedGlassComponents example">
      ThemedGlassComponents content
    </ThemedGlassComponents>
  );
}
```
