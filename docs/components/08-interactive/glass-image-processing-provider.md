...
- Category: misc
- Direct Storybook story: `src/components/image/GlassImageProcessingProvider.stories.tsx`
...
```tsx
import { GlassImageProcessingProvider } from 'aura-glass';

export function GlassImageProcessingProviderExample() {
  return (
    <GlassImageProcessingProvider aria-label="GlassImageProcessingProvider example">
      GlassImageProcessingProvider content
    </GlassImageProcessingProvider>
  );
}
```
