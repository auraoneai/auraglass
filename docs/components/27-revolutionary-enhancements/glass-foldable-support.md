...
```tsx
import { GlassFoldableSupport } from 'aura-glass';

export function GlassFoldableSupportExample() {
  return (
    <GlassFoldableSupport aria-label="GlassFoldableSupport example">
      GlassFoldableSupport content
    </GlassFoldableSupport>
  );
}
```
