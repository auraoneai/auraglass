...
```tsx
import { GlassDragDropProvider } from 'aura-glass';

export function GlassDragDropProviderExample() {
  return (
    <GlassDragDropProvider aria-label="GlassDragDropProvider example">
      GlassDragDropProvider content
    </GlassDragDropProvider>
  );
}
```
