...
```tsx
import { GlassReactionBubbles } from 'aura-glass';

export function GlassReactionBubblesExample() {
  return (
    <GlassReactionBubbles aria-label="GlassReactionBubbles example">
      GlassReactionBubbles content
    </GlassReactionBubbles>
  );
}
```
