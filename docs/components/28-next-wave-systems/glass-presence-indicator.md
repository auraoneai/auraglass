...
```tsx
import { GlassPresenceIndicator } from 'aura-glass';

export function GlassPresenceIndicatorExample() {
  return (
    <GlassPresenceIndicator aria-label="GlassPresenceIndicator example">
      GlassPresenceIndicator content
    </GlassPresenceIndicator>
  );
}
```
