...
- Category: misc
- Direct Storybook story: `src/components/social/GlassVoiceWaveform.stories.tsx`
...
```tsx
import { GlassVoiceWaveform } from 'aura-glass';

export function GlassVoiceWaveformExample() {
  return (
    <GlassVoiceWaveform aria-label="GlassVoiceWaveform example">
      GlassVoiceWaveform content
    </GlassVoiceWaveform>
  );
}
```
