...
```tsx
import { GlassBiomeSimulator } from 'aura-glass';

export function GlassBiomeSimulatorExample() {
  return (
    <GlassBiomeSimulator aria-label="GlassBiomeSimulator example">
      GlassBiomeSimulator content
    </GlassBiomeSimulator>
  );
}
```
