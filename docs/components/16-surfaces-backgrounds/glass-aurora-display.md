...
- Category: misc
- Direct Storybook story: `src/components/atmospheric/GlassAuroraDisplay.stories.tsx`
...
```tsx
import { GlassAuroraDisplay } from 'aura-glass';

export function GlassAuroraDisplayExample() {
  return (
    <GlassAuroraDisplay aria-label="GlassAuroraDisplay example">
      GlassAuroraDisplay content
    </GlassAuroraDisplay>
  );
}
```
