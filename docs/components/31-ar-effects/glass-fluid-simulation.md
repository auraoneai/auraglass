...
```tsx
import { GlassFluidSimulation } from 'aura-glass';

export function GlassFluidSimulationExample() {
  return (
    <GlassFluidSimulation aria-label="GlassFluidSimulation example">
      GlassFluidSimulation content
    </GlassFluidSimulation>
  );
}
```
