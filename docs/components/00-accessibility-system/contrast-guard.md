...
```tsx
import { ContrastGuard } from 'aura-glass';

export function ContrastGuardExample() {
  return (
    <ContrastGuard aria-label="ContrastGuard example">
      ContrastGuard content
    </ContrastGuard>
  );
}
```
