...
```tsx
import { GlassMusicVisualizer } from 'aura-glass';

export function GlassMusicVisualizerExample() {
  return (
    <GlassMusicVisualizer aria-label="GlassMusicVisualizer example">
      GlassMusicVisualizer content
    </GlassMusicVisualizer>
  );
}
```
