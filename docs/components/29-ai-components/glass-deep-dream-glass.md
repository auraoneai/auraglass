...
```tsx
import { GlassDeepDreamGlass } from 'aura-glass';

export function GlassDeepDreamGlassExample() {
  return (
    <GlassDeepDreamGlass aria-label="GlassDeepDreamGlass example">
      GlassDeepDreamGlass content
    </GlassDeepDreamGlass>
  );
}
```
