...
```tsx
import { GlobalCookieConsent } from 'aura-glass';

export function GlobalCookieConsentExample() {
  return (
    <GlobalCookieConsent aria-label="GlobalCookieConsent example">
      GlobalCookieConsent content
    </GlobalCookieConsent>
  );
}
```
