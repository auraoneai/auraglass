...
```tsx
import { CompactCookieNotice } from 'aura-glass';

export function CompactCookieNoticeExample() {
  return (
    <CompactCookieNotice aria-label="CompactCookieNotice example">
      CompactCookieNotice content
    </CompactCookieNotice>
  );
}
```
