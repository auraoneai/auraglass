...
- Category: misc
- Direct Storybook story: `src/components/advanced/LiquidGlassGPU.stories.tsx`
...
```tsx
import { LiquidGlassGPU } from 'aura-glass';

export function LiquidGlassGPUExample() {
  return (
    <LiquidGlassGPU aria-label="LiquidGlassGPU example">
      LiquidGlassGPU content
    </LiquidGlassGPU>
  );
}
```
