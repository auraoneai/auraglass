...
- `src/components/status/GlassConnectionStatus.tsx`
...
```tsx
import { GlassConnectionStatus } from 'aura-glass';

export function GlassConnectionStatusExample() {
  return (
    <GlassConnectionStatus aria-label="GlassConnectionStatus example">
      GlassConnectionStatus content
    </GlassConnectionStatus>
  );
}
```
