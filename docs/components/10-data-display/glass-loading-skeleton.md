...
- Direct Storybook story: `src/components/data-display/GlassLoadingSkeleton.stories.tsx`
...
```tsx
import { GlassLoadingSkeleton } from 'aura-glass';

export function GlassLoadingSkeletonExample() {
  return (
    <GlassLoadingSkeleton aria-label="GlassLoadingSkeleton example">
      GlassLoadingSkeleton content
    </GlassLoadingSkeleton>
  );
}
```
