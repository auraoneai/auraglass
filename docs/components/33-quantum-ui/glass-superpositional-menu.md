...
```tsx
import { GlassSuperpositionalMenu } from 'aura-glass';

export function GlassSuperpositionalMenuExample() {
  return (
    <GlassSuperpositionalMenu aria-label="GlassSuperpositionalMenu example">
      GlassSuperpositionalMenu content
    </GlassSuperpositionalMenu>
  );
}
```
