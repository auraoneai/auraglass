...
- Category: chart
- Direct Storybook story: `src/components/charts/ModularGlassDataChart.stories.tsx`
...
```tsx
import { ModularGlassDataChart } from 'aura-glass';

export function ModularGlassDataChartExample() {
  return (
    <ModularGlassDataChart aria-label="ModularGlassDataChart example">
      ModularGlassDataChart content
    </ModularGlassDataChart>
  );
}
```
